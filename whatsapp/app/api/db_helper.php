...
function fetch_select($tb,$text,$fk_column = ''){
    $pk = getPk($tb);
    $data = format_output(executeQuery("SELECT $pk, $text FROM `$tb`")['data'],'<select class="w-full p-2 dark:border-gray-600"><option>Choose '.htmlentities(ucfirst(str_replace('_',' ',$tb))).'</option>@for <option value="*'.$pk.'">*'.$text.'</option> @end</select>');
    return $data;
}
