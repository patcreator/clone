...
include_once '../../../system/cogs/functions.php';$row = fetchData("SELECT * FROM `pdt_provincecoordinator` WHERE `id` = ?", [$id])[0] ?? null;
if (!$row) { die('Record not found'); }
?>
<div class="max-w-3xl mx-auto mt-8 bg-white p-6 rounded-lg shadow-md"><form id="editpdt_provincecoordinatorForm" method="POST" action="/update/pdt_provincecoordinator?id=<?= $id ?>" class="space-y-6">
            <div>
...
<?php endforeach; ?>
        ...
        <div>
            <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 transition">
                Update Pdt_provincecoordinator
            </button>
        </div>
    </form></div>