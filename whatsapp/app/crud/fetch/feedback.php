...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">feedback</h1>
    ...
    <!-- Add feedback Button -->
    <a href="#" id="addfeedbackBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add feedback
    </a>

    <!-- Reusable Modal -->
    <div id="feedbackModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">feedback</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="feedbackTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">name</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">subject</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">rating</th>
    ...
    <?php foreach($feedback as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['name']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['name']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="feedbackTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['name']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['email']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['subject']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['message']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['rating']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/feedback.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All feedback</a>
</div>
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/feedback.php', {
  ...
  function openModal(title,content){
    ...
    $('#feedbackModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });
  ...
  $('#addfeedbackBtn').click(function(e){
    ...
    $.post('<?= $path?>app/crud/forms/create/feedback.php', function(res){
      if(res){
        openModal('Add feedback',res);
  ...
  $(document).on('submit','#addfeedbackForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/feedback.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#feedbackTable").load(location.href + " #feedbackTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/feedback.php', $('#addfeedbackForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#feedbackTable").load(location.href + " #feedbackTable");
        window.location.reload();
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/feedback.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>user_id:</strong> ${res.data.user_id}</p><p class="text-gray-700 dark:text-gray-200"><strong>name:</strong> ${res.data.name}</p><p class="text-gray-700 dark:text-gray-200"><strong>email:</strong> ${res.data.email}</p><p class="text-gray-700 dark:text-gray-200"><strong>subject:</strong> ${res.data.subject}</p><p class="text-gray-700 dark:text-gray-200"><strong>message:</strong> ${res.data.message}</p><p class="text-gray-700 dark:text-gray-200"><strong>rating:</strong> ${res.data.rating}</p><p class="text-gray-700 dark:text-gray-200"><strong>type:</strong> ${res.data.type}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View feedback', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/feedback.php', {id}, function(res){
        openModal('Edit feedback',res);
  ...
  $(document).on('submit','#editfeedbackForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/feedback.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#feedbackTable").load(location.href + " #feedbackTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/feedback.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#feedbackTable").load(location.href + " #feedbackTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each feedback data
        $.post('<?= $path?>app/crud/api/feedback.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(feedback => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy feedback</h3>
                            <input type="hidden" name="ids[]" value="${feedback.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${feedback.id}]" value="${feedback.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="user_id">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${feedback.id}]" value="${feedback.user_id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="name">name</label>
                        <input type="name" id="name" name="name[${feedback.id}]" value="${feedback.name}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${feedback.id}]" value="${feedback.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="subject">subject</label>
                        <input type="subject" id="subject" name="subject[${feedback.id}]" value="${feedback.subject}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="message">message</label>
                        <input type="message" id="message" name="message[${feedback.id}]" value="${feedback.message}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="rating">rating</label>
                        <input type="rating" id="rating" name="rating[${feedback.id}]" value="${feedback.rating}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="type">type</label>
                        <input type="type" id="type" name="type[${feedback.id}]" value="${feedback.type}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${feedback.id}]" value="${feedback.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${feedback.id}]" value="${feedback.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${feedback.id}]" value="${feedback.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy feedback', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected feedback?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(feedback => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${feedback.id}</p><p>user_id: ${feedback.user_id}</p><p>name: ${feedback.name}</p><p>email: ${feedback.email}</p><p>subject: ${feedback.subject}</p><p>message: ${feedback.message}</p><p>rating: ${feedback.rating}</p><p>type: ${feedback.type}</p><p>created_at: ${feedback.created_at}</p><p>updated_at: ${feedback.updated_at}</p><p>status: ${feedback.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/feedback.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each feedback's data
        $.post('<?= $path?>app/crud/api/feedback.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each feedback
                let html = '';
                res.data.forEach(feedback => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit feedback</h3>
                            <input type="hidden" name="ids[]" value="${feedback.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${feedback.id}]" value="${feedback.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="user_id" class="block text-gray-700 dark:text-gray-200">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${feedback.id}]" value="${feedback.user_id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="name" class="block text-gray-700 dark:text-gray-200">name</label>
                        <input type="name" id="name" name="name[${feedback.id}]" value="${feedback.name}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${feedback.id}]" value="${feedback.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="subject" class="block text-gray-700 dark:text-gray-200">subject</label>
                        <input type="subject" id="subject" name="subject[${feedback.id}]" value="${feedback.subject}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="message" class="block text-gray-700 dark:text-gray-200">message</label>
                        <input type="message" id="message" name="message[${feedback.id}]" value="${feedback.message}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="rating" class="block text-gray-700 dark:text-gray-200">rating</label>
                        <input type="rating" id="rating" name="rating[${feedback.id}]" value="${feedback.rating}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="type" class="block text-gray-700 dark:text-gray-200">type</label>
                        <input type="type" id="type" name="type[${feedback.id}]" value="${feedback.type}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${feedback.id}]" value="${feedback.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${feedback.id}]" value="${feedback.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${feedback.id}]" value="${feedback.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit feedback', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/feedback.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#feedbackTable").load(location.href + " #feedbackTable");
...
// Collect all feedback fields
const excluded = ['id', 'user_id', 'name', 'email', 'subject', 'message', 'rating', 'type', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/feedback.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#feedbackModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#feedbackTable").load(location.href + " #feedbackTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/feedback.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#feedbackTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#feedbackTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
