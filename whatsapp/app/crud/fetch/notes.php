...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">notes</h1>
    ...
    <!-- Add notes Button -->
    <a href="#" id="addnotesBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add notes
    </a>

    <!-- Reusable Modal -->
    <div id="notesModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">notes</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="notesTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="notesTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/notes.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All notes</a>
</div>
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'notes Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/notes.php', {
  ...
  function openModal(title,content){
    ...
    $('#notesModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });
  ...
  $('#addnotesBtn').click(function(e){
    ...
    $.post('<?= $path?>app/crud/forms/create/notes.php', function(res){
      if(res){
        openModal('Add notes',res);
  ...
  $(document).on('submit','#addnotesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/notes.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#notesTable").load(location.href + " #notesTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/notes.php', $('#addnotesForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#notesTable").load(location.href + " #notesTable");
        window.location.reload();
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/notes.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/notes.php', {id}, function(res){
        openModal('Edit notes',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editnotesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/notes.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#notesTable").load(location.href + " #notesTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/notes.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#notesTable").load(location.href + " #notesTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each notes data
        $.post('<?= $path?>app/crud/api/notes.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(notes => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy notes</h3>
                            <input type="hidden" name="ids[]" value="${notes.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${notes.id}]" value="${notes.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="title">title</label>
                        <input type="title" id="title" name="title[${notes.id}]" value="${notes.title}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="content">content</label>
                        <input type="content" id="content" name="content[${notes.id}]" value="${notes.content}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${notes.id}]" value="${notes.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${notes.id}]" value="${notes.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${notes.id}]" value="${notes.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy notes', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected notes?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(notes => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${notes.id}</p><p>title: ${notes.title}</p><p>content: ${notes.content}</p><p>created_at: ${notes.created_at}</p><p>updated_at: ${notes.updated_at}</p><p>status: ${notes.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/notes.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each notes's data
        $.post('<?= $path?>app/crud/api/notes.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each notes
                let html = '';
                res.data.forEach(notes => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit notes</h3>
                            <input type="hidden" name="ids[]" value="${notes.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${notes.id}]" value="${notes.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="title" class="block text-gray-700 dark:text-gray-200">title</label>
                        <input type="title" id="title" name="title[${notes.id}]" value="${notes.title}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="content" class="block text-gray-700 dark:text-gray-200">content</label>
                        <input type="content" id="content" name="content[${notes.id}]" value="${notes.content}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${notes.id}]" value="${notes.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${notes.id}]" value="${notes.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${notes.id}]" value="${notes.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit notes', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/notes.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#notesTable").load(location.href + " #notesTable");
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/notes.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#notesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#notesTable").load(location.href + " #notesTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/notes.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#notesTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#notesTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
