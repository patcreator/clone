<?php
  $stmt = $pdo->query("SELECT * FROM pdt_provincecoordinator");
  $pdt_provincecoordinator = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
foreach ($pdt_provincecoordinator as $item) {
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">pdt_provincecoordinator</h1>
    ...
    <!-- Add pdt_provincecoordinator Button -->
    <a href="#" id="addpdt_provincecoordinatorBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add pdt_provincecoordinator
    </a>

    <!-- Reusable Modal -->
    <div id="pdt_provincecoordinatorModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">pdt_provincecoordinator</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="pdt_provincecoordinatorTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">created_at</th>
          ...
          <tbody>
            <?php $i=1; foreach($pdt_provincecoordinator as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="pdt_provincecoordinatorChart" height="120"></canvas>
</div>

<!-- Grid Tab -->
<div id="tab-grid" class="hidden px-8 pt-4">
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    <?php foreach($pdt_provincecoordinator as $item): ?>
      ...
      >
        <img
          ...
          alt="pdt_provincecoordinator Image"
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          <!-- Optional pdt_provincecoordinator Image -->
          <img
            src="<?= htmlspecialchars($item['image'] ?? 'https://placehold.co/600x400') ?>"
            alt="Image"
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="pdt_provincecoordinatorTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/pdt_provincecoordinator.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All pdt_provincecoordinator</a>
</div>
...
const ctx = document.getElementById('pdt_provincecoordinatorChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'pdt_provincecoordinator Created',
        data: [
          <?= $counts['today'] ?>,
          <?= $counts['week'] ?>,
          <?= $counts['month'] ?>,
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'pdt_provincecoordinator Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php', {
  ...
  function openModal(title,content){
    ...
    $('#pdt_provincecoordinatorModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add pdt_provincecoordinator ---
  $('#addpdt_provincecoordinatorBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/pdt_provincecoordinator.php', function(res){
      if(res){
        openModal('Add pdt_provincecoordinator',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addpdt_provincecoordinatorForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php', $('#addpdt_provincecoordinatorForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
        window.location.reload();
        $('#addpdt_provincecoordinatorForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });

// --- View pdt_provincecoordinator ---
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          <div class="bg-blue-500 text-white p-4 rounded mb-4 text-lg font-semibold">
            pdt_provincecoordinator
          </div>
        ...
        `;
        openModal('View pdt_provincecoordinator', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});


  // --- Edit pdt_provincecoordinator ---
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/pdt_provincecoordinator.php', {id}, function(res){
        openModal('Edit pdt_provincecoordinator',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editpdt_provincecoordinatorForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
  ...
  // --- Delete pdt_provincecoordinator ---
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this pdt_provincecoordinator?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each pdt_provincecoordinator data
        $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(pdt_provincecoordinator => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy pdt_provincecoordinator</h3>
                            <input type="hidden" name="ids[]" value="${pdt_provincecoordinator.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="fullName">fullName</label>
                        <input type="fullName" id="fullName" name="fullName[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.fullName}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="phone">phone</label>
                        <input type="phone" id="phone" name="phone[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.phone}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="password">password</label>
                        <input type="password" id="password" name="password[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.password}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="createdDate">createdDate</label>
                        <input type="createdDate" id="createdDate" name="createdDate[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.createdDate}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.status}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="Level">Level</label>
                        <input type="Level" id="Level" name="Level[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.Level}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="Location">Location</label>
                        <input type="Location" id="Location" name="Location[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.Location}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.created_at}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy pdt_provincecoordinator', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected pdt_provincecoordinator?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(pdt_provincecoordinator => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${pdt_provincecoordinator.id}</p><p>fullName: ${pdt_provincecoordinator.fullName}</p><p>email: ${pdt_provincecoordinator.email}</p><p>phone: ${pdt_provincecoordinator.phone}</p><p>password: ${pdt_provincecoordinator.password}</p><p>createdDate: ${pdt_provincecoordinator.createdDate}</p><p>status: ${pdt_provincecoordinator.status}</p><p>Level: ${pdt_provincecoordinator.Level}</p><p>Location: ${pdt_provincecoordinator.Location}</p><p>created_at: ${pdt_provincecoordinator.created_at}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/pdt_provincecoordinator.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each pdt_provincecoordinator's data
        $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each pdt_provincecoordinator
                let html = '';
                res.data.forEach(pdt_provincecoordinator => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit pdt_provincecoordinator</h3>
                            <input type="hidden" name="ids[]" value="${pdt_provincecoordinator.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="fullName" class="block text-gray-700 dark:text-gray-200">fullName</label>
                        <input type="fullName" id="fullName" name="fullName[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.fullName}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="phone" class="block text-gray-700 dark:text-gray-200">phone</label>
                        <input type="phone" id="phone" name="phone[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.phone}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="password" class="block text-gray-700 dark:text-gray-200">password</label>
                        <input type="password" id="password" name="password[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.password}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="createdDate" class="block text-gray-700 dark:text-gray-200">createdDate</label>
                        <input type="createdDate" id="createdDate" name="createdDate[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.createdDate}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="Level" class="block text-gray-700 dark:text-gray-200">Level</label>
                        <input type="Level" id="Level" name="Level[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.Level}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="Location" class="block text-gray-700 dark:text-gray-200">Location</label>
                        <input type="Location" id="Location" name="Location[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.Location}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${pdt_provincecoordinator.id}]" value="${pdt_provincecoordinator.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit pdt_provincecoordinator', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
...
// Collect all pdt_provincecoordinator fields
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/pdt_provincecoordinator.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#pdt_provincecoordinatorModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#pdt_provincecoordinatorTable").load(location.href + " #pdt_provincecoordinatorTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/pdt_provincecoordinator.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#pdt_provincecoordinatorTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#pdt_provincecoordinatorTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#pdt_provincecoordinatorTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
