...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">contacts</h1>
    ...
    <!-- Add contacts Button -->
    <a href="#" id="addcontactsBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add contacts
    </a>

    <!-- Reusable Modal -->
    <div id="contactsModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">contacts</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="contactsTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($contacts as $item): ?>
    ...
    <?php foreach($contacts as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['phone']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['phone']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="contactsTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/contacts.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All contacts</a>
</div>
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'contacts Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/contacts.php', {
  ...
  function openModal(title,content){
    ...
    $('#contactsModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#contactsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add contacts ---
  $('#addcontactsBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/contacts.php', function(res){
      if(res){
        openModal('Add contacts',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addcontactsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/contacts.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/contacts.php', $('#addcontactsForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#contactsTable").load(location.href + " #contactsTable");
        window.location.reload();
        $('#addcontactsForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/contacts.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>name:</strong> ${res.data.name}</p><p class="text-gray-700 dark:text-gray-200"><strong>phone:</strong> ${res.data.phone}</p><p class="text-gray-700 dark:text-gray-200"><strong>email:</strong> ${res.data.email}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View contacts', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/contacts.php', {id}, function(res){
        openModal('Edit contacts',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editcontactsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/contacts.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#contactsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#contactsTable").load(location.href + " #contactsTable");
  ...
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this contacts?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/contacts.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#contactsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#contactsTable").load(location.href + " #contactsTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each contacts data
        $.post('<?= $path?>app/crud/api/contacts.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(contacts => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy contacts</h3>
                            <input type="hidden" name="ids[]" value="${contacts.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${contacts.id}]" value="${contacts.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="name">name</label>
                        <input type="name" id="name" name="name[${contacts.id}]" value="${contacts.name}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="phone">phone</label>
                        <input type="phone" id="phone" name="phone[${contacts.id}]" value="${contacts.phone}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${contacts.id}]" value="${contacts.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${contacts.id}]" value="${contacts.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${contacts.id}]" value="${contacts.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${contacts.id}]" value="${contacts.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy contacts', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected contacts?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(contacts => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${contacts.id}</p><p>name: ${contacts.name}</p><p>phone: ${contacts.phone}</p><p>email: ${contacts.email}</p><p>created_at: ${contacts.created_at}</p><p>updated_at: ${contacts.updated_at}</p><p>status: ${contacts.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/contacts.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each contacts's data
        $.post('<?= $path?>app/crud/api/contacts.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each contacts
                let html = '';
                res.data.forEach(contacts => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit contacts</h3>
                            <input type="hidden" name="ids[]" value="${contacts.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${contacts.id}]" value="${contacts.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="name" class="block text-gray-700 dark:text-gray-200">name</label>
                        <input type="name" id="name" name="name[${contacts.id}]" value="${contacts.name}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="phone" class="block text-gray-700 dark:text-gray-200">phone</label>
                        <input type="phone" id="phone" name="phone[${contacts.id}]" value="${contacts.phone}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${contacts.id}]" value="${contacts.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${contacts.id}]" value="${contacts.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${contacts.id}]" value="${contacts.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${contacts.id}]" value="${contacts.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit contacts', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/contacts.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#contactsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#contactsTable").load(location.href + " #contactsTable");
...
// Collect all contacts fields
const excluded = ['id', 'name', 'phone', 'email', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/contacts.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#contactsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#contactsTable").load(location.href + " #contactsTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/contacts.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#contactsTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#contactsTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#contactsTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
