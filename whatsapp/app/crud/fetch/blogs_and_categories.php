                ...
                <?php
  $stmt = $pdo->query("SELECT * FROM blogs_and_categories");
  $blogs_and_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
foreach ($blogs_and_categories as $item) {
...
$columns = ['BAC_ID','blog','category','created_at'];
?>
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">blogs_and_categories</h1>
    ...
    <!-- Add blogs_and_categories Button -->
    <a href="#" id="addblogs_and_categoriesBtn" class="mb-4 ml-8 px-4 py-2 bg-red-500 text-white rounded inline-block">
      Add blogs_and_categories
    </a>

    <!-- Reusable Modal -->
    <div id="blogs_and_categoriesModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">blogs_and_categories</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="blogs_and_categoriesTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
          ...
          <tbody>
            <?php $i=1; foreach($blogs_and_categories as $item): ?>
            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 hover:dark:bg-gray-700">
                <td class="border-0 px-4 py-4 dark:bg-gray-800"><input type="checkbox" class="rowCheckbox" data-id="<?= $item['BAC_ID'] ?>"></td>

                <td class="border-0 px-4 py-4 dark:bg-gray-800"><?= $i++ ?></td>

                 
                    <td class="editable" data-id="<?= $item['BAC_ID'] ?>" data-field="BAC_ID" data-values=''><?= htmlspecialchars($item['BAC_ID']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['BAC_ID'] ?>" data-field="blog" data-values=''><?= htmlspecialchars($item['blog']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['BAC_ID'] ?>" data-field="category" data-values=''><?= htmlspecialchars($item['category']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['BAC_ID'] ?>" data-field="created_at" data-values=''><?= htmlspecialchars($item['created_at']) ?></td>
                    
                <td class="border-0 px-4 py-4 dark:bg-gray-800">
                  <button class="viewBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['BAC_ID'] ?>" title="view"><i class="mdi-24px mdi mdi-eye"></i></button>
                  <button class="editBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['BAC_ID'] ?>" title="edit"><i class="mdi-24px mdi mdi-pen"></i></button>
                  <button class="deleteBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['BAC_ID'] ?>" title="delete"><i class="mdi-24px mdi mdi-delete"></i></button>
                </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="blogs_and_categoriesChart" height="120"></canvas>
</div>

<!-- Grid Tab -->
<div id="tab-grid" class="hidden px-8 pt-4">
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    <?php foreach($blogs_and_categories as $key => $item): ?>
      <div
      ...
      >
        <img
          ...
          alt="blogs_and_categories Image"
          class="w-full h-40 object-cover"
        />
        <div class="p-4">
          <h3 class="text-lg font-bold text-red-600"><?= htmlspecialchars($item['BAC_ID']) ?></h3>
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['blog']??'') ?></p>
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['category']??'') ?></p>
...
<div id="tab-list" class="hidden px-8 pt-4">
  <ul class="space-y-4">
    <?php foreach($blogs_and_categories as $item): ?>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-red-500 transition">
        <div class="flex items-center space-x-4">
          <!-- Optional blogs_and_categories Image -->
          <img
            src="<?= htmlspecialchars($item['image'] ?? 'https://placehold.co/600x400') ?>"
            alt="Image"
          ...
          <!-- blogs_and_categories Info -->
          <div>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['BAC_ID']) ?></p>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['blog']) ?></p>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['category']) ?></p>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/blogs_and_categories.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All blogs_and_categories</a>
</div>
...
const ctx = document.getElementById('blogs_and_categoriesChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'blogs_and_categories Created',
        data: [
          <?= $counts['today'] ?>,
          <?= $counts['week'] ?>,
          <?= $counts['month'] ?>,
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'blogs_and_categories Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/blogs_and_categories.php', {
  ...
  function openModal(title,content){
    ...
    $('#blogs_and_categoriesModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add blogs_and_categories ---
  $('#addblogs_and_categoriesBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/blogs_and_categories.php', function(res){
      if(res){
        openModal('Add blogs_and_categories',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addblogs_and_categoriesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php', $('#addblogs_and_categoriesForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
        window.location.reload();
        $('#addblogs_and_categoriesForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });

// --- View blogs_and_categories ---
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>BAC_ID:</strong> ${res.data.BAC_ID}</p> <p class="text-gray-700 dark:text-gray-200"><strong>blog:</strong> ${res.data.blog}</p> <p class="text-gray-700 dark:text-gray-200"><strong>category:</strong> ${res.data.category}</p> <p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p> 
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-red-500 text-white px-6 py-2 rounded hover:bg-red-600">Close</button>
          </div>
        `;
        openModal('View blogs_and_categories', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});


  // --- Edit blogs_and_categories ---
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/blogs_and_categories.php', {id}, function(res){
        openModal('Edit blogs_and_categories',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editblogs_and_categoriesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
  ...
  // --- Delete blogs_and_categories ---
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this blogs_and_categories?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each blogs_and_categories data
        $.post('<?= $path?>app/crud/api/blogs_and_categories.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(blogs_and_categories => {
                    html += `
                    <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                        <h3 class="font-bold text-lg mb-2 text-green-500">Copy blogs_and_categories</h3>
                        <input type="hidden" name="ids[]" value="${blogs_and_categories.id}">
                        <div class="grid grid-cols-2 gap-4">
                        
                            <div>
                                <label for="BAC_ID">BAC_ID</label>
                                <input type="text" id="BAC_ID" name="BAC_ID[${blogs_and_categories.id}]" value="${blogs_and_categories.BAC_ID}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="blog">blog</label>
                                <input type="text" id="blog" name="blog[${blogs_and_categories.id}]" value="${blogs_and_categories.blog}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="category">category</label>
                                <input type="text" id="category" name="category[${blogs_and_categories.id}]" value="${blogs_and_categories.category}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="created_at">created_at</label>
                                <input type="text" id="created_at" name="created_at[${blogs_and_categories.id}]" value="${blogs_and_categories.created_at}" class="w-full p-2 border rounded mb-2">
                            </div> 
                        </div>
                    </div>
                    `;
                ...
                openModal('Bulk Copy blogs_and_categories', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected blogs_and_categories?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(blogs_and_categories => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                        ...
                                        <p>BAC_ID: ${blogs_and_categories.BAC_ID}</p>
                                    
                                        <p>blog: ${blogs_and_categories.blog}</p>
                                    
                                        <p>category: ${blogs_and_categories.category}</p>
                                    
                                        <p>created_at: ${blogs_and_categories.created_at}</p>
          ...
          window.location.href = `<?= $path?>app/crud/api/blogs_and_categories.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each blogs_and_categories's data
        $.post('<?= $path?>app/crud/api/blogs_and_categories.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each blogs_and_categories
                let html = '';
                res.data.forEach(blogs_and_categories => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-red-500">Edit blogs_and_categories</h3>
                            <input type="hidden" name="ids[]" value="${blogs_and_categories.id}">
                            <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" id="created_at" name="created_at[${blogs_and_categories.id}]" value="${blogs_and_categories.created_at}" class="w-full p-2 border rounded mb-2" required>
                                        </div> 
                ...
                openModal('Bulk Edit blogs_and_categories', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
...
// Collect all blogs_and_categories fields
const excluded = [
    'BAC_ID',
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/blogs_and_categories.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#blogs_and_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#blogs_and_categoriesTable").load(location.href + " #blogs_and_categoriesTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/blogs_and_categories.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#blogs_and_categoriesTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#blogs_and_categoriesTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#blogs_and_categoriesTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
