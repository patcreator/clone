<?php
  $stmt = $pdo->query("SELECT * FROM search_index");
  $search_index = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">search_index</h1>
    ...
    <!-- Add search_index Button -->
    <a href="#" id="addsearch_indexBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add search_index
    </a>

    <!-- Reusable Modal -->
    <div id="search_indexModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">search_index</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="search_indexTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">content</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">keywords</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">url</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($search_index as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="search_indexChart" height="120"></canvas>
</div>
    ...
    <?php foreach($search_index as $item): ?>
      ...
      >
        <img
          ...
          alt="search_index Image"
          class="w-full h-40 object-cover"
        />
        <div class="p-4">
          <h3 class="text-lg font-bold text-blue-600"><?= htmlspecialchars($item['id']) ?></h3>
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['entity_type']??'') ?></p>
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['entity_id']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['entity_type']) ?></p>
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['entity_id']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="search_indexTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['content']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['keywords']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['url']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/search_index.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All search_index</a>
</div>
...
const ctx = document.getElementById('search_indexChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'search_index Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/search_index.php', {
  ...
  function openModal(title,content){
    ...
    $('#search_indexModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add search_index ---
  $('#addsearch_indexBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/search_index.php', function(res){
      if(res){
        openModal('Add search_index',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addsearch_indexForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/search_index.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#search_indexTable").load(location.href + " #search_indexTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/search_index.php', $('#addsearch_indexForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#search_indexTable").load(location.href + " #search_indexTable");
        window.location.reload();
        $('#addsearch_indexForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/search_index.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>entity_type:</strong> ${res.data.entity_type}</p><p class="text-gray-700 dark:text-gray-200"><strong>entity_id:</strong> ${res.data.entity_id}</p><p class="text-gray-700 dark:text-gray-200"><strong>title:</strong> ${res.data.title}</p><p class="text-gray-700 dark:text-gray-200"><strong>content:</strong> ${res.data.content}</p><p class="text-gray-700 dark:text-gray-200"><strong>keywords:</strong> ${res.data.keywords}</p><p class="text-gray-700 dark:text-gray-200"><strong>url:</strong> ${res.data.url}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View search_index', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/search_index.php', {id}, function(res){
        openModal('Edit search_index',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editsearch_indexForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/search_index.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#search_indexTable").load(location.href + " #search_indexTable");
  ...
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this search_index?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/search_index.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#search_indexTable").load(location.href + " #search_indexTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each search_index data
        $.post('<?= $path?>app/crud/api/search_index.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(search_index => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy search_index</h3>
                            <input type="hidden" name="ids[]" value="${search_index.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${search_index.id}]" value="${search_index.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="entity_type">entity_type</label>
                        <input type="entity_type" id="entity_type" name="entity_type[${search_index.id}]" value="${search_index.entity_type}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="entity_id">entity_id</label>
                        <input type="entity_id" id="entity_id" name="entity_id[${search_index.id}]" value="${search_index.entity_id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="title">title</label>
                        <input type="title" id="title" name="title[${search_index.id}]" value="${search_index.title}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="content">content</label>
                        <input type="content" id="content" name="content[${search_index.id}]" value="${search_index.content}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="keywords">keywords</label>
                        <input type="keywords" id="keywords" name="keywords[${search_index.id}]" value="${search_index.keywords}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="url">url</label>
                        <input type="url" id="url" name="url[${search_index.id}]" value="${search_index.url}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${search_index.id}]" value="${search_index.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${search_index.id}]" value="${search_index.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${search_index.id}]" value="${search_index.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy search_index', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected search_index?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(search_index => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${search_index.id}</p><p>entity_type: ${search_index.entity_type}</p><p>entity_id: ${search_index.entity_id}</p><p>title: ${search_index.title}</p><p>content: ${search_index.content}</p><p>keywords: ${search_index.keywords}</p><p>url: ${search_index.url}</p><p>created_at: ${search_index.created_at}</p><p>updated_at: ${search_index.updated_at}</p><p>status: ${search_index.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/search_index.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each search_index's data
        $.post('<?= $path?>app/crud/api/search_index.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each search_index
                let html = '';
                res.data.forEach(search_index => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit search_index</h3>
                            <input type="hidden" name="ids[]" value="${search_index.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${search_index.id}]" value="${search_index.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="entity_type" class="block text-gray-700 dark:text-gray-200">entity_type</label>
                        <input type="entity_type" id="entity_type" name="entity_type[${search_index.id}]" value="${search_index.entity_type}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="entity_id" class="block text-gray-700 dark:text-gray-200">entity_id</label>
                        <input type="entity_id" id="entity_id" name="entity_id[${search_index.id}]" value="${search_index.entity_id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="title" class="block text-gray-700 dark:text-gray-200">title</label>
                        <input type="title" id="title" name="title[${search_index.id}]" value="${search_index.title}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="content" class="block text-gray-700 dark:text-gray-200">content</label>
                        <input type="content" id="content" name="content[${search_index.id}]" value="${search_index.content}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="keywords" class="block text-gray-700 dark:text-gray-200">keywords</label>
                        <input type="keywords" id="keywords" name="keywords[${search_index.id}]" value="${search_index.keywords}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="url" class="block text-gray-700 dark:text-gray-200">url</label>
                        <input type="url" id="url" name="url[${search_index.id}]" value="${search_index.url}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${search_index.id}]" value="${search_index.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${search_index.id}]" value="${search_index.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${search_index.id}]" value="${search_index.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit search_index', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/search_index.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#search_indexTable").load(location.href + " #search_indexTable");
...
// Collect all search_index fields
const excluded = ['id', 'entity_type', 'entity_id', 'title', 'content', 'keywords', 'url', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/search_index.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#search_indexModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#search_indexTable").load(location.href + " #search_indexTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/search_index.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#search_indexTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#search_indexTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#search_indexTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
