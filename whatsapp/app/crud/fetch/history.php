...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">history</h1>
    ...
    <!-- Add history Button -->
    <a href="#" id="addhistoryBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add history
    </a>

    <!-- Reusable Modal -->
    <div id="historyModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">history</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="historyTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">action</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">entity_type</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">entity_id</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">old_value</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">new_value</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">ip_address</th>
 
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">user_agent</th>
          ...
          <tbody>
            ...
            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 hover:dark:bg-gray-700">
                ...
                <td class="border-0 px-4 py-4 dark:bg-gray-800"><?= $i++ ?></td>
                 <td class="editable" data-id="<?= $item['id'] ?>" data-field="id" data-values='20'><?= htmlspecialchars($item['id']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="user_id" data-values='20'><?= htmlspecialchars($item['user_id']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="action" data-values='255'><?= htmlspecialchars($item['action']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="entity_type" data-values='100'><?= htmlspecialchars($item['entity_type']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="entity_id" data-values='20'><?= htmlspecialchars($item['entity_id']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="old_value" data-values=''><?= htmlspecialchars($item['old_value']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="new_value" data-values=''><?= htmlspecialchars($item['new_value']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="ip_address" data-values='50'><?= htmlspecialchars($item['ip_address']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="user_agent" data-values='255'><?= htmlspecialchars($item['user_agent']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="created_at" data-values=''><?= htmlspecialchars($item['created_at']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="updated_at" data-values=''><?= htmlspecialchars($item['updated_at']) ?></td> <td class="editable" data-id="<?= $item['id'] ?>" data-field="status" data-values='<option>active</option><option>not-active</option><option>not active</option>'><?= htmlspecialchars($item['status']) ?></td>
    ...
    <?php foreach($history as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['action']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['action']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="historyTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['action']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['entity_type']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['entity_id']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['old_value']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['new_value']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['ip_address']) ?></td>
 
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['user_agent']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/history.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All history</a>
</div>
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'history Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/history.php', {
  ...
  function openModal(title,content){
    ...
    $('#historyModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#historyModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });
  ...
  $('#addhistoryBtn').click(function(e){
    ...
    $.post('<?= $path?>app/crud/forms/create/history.php', function(res){
      if(res){
        openModal('Add history',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addhistoryForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/history.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/history.php', $('#addhistoryForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#historyTable").load(location.href + " #historyTable");
        window.location.reload();
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/history.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>user_id:</strong> ${res.data.user_id}</p><p class="text-gray-700 dark:text-gray-200"><strong>action:</strong> ${res.data.action}</p><p class="text-gray-700 dark:text-gray-200"><strong>entity_type:</strong> ${res.data.entity_type}</p><p class="text-gray-700 dark:text-gray-200"><strong>entity_id:</strong> ${res.data.entity_id}</p><p class="text-gray-700 dark:text-gray-200"><strong>old_value:</strong> ${res.data.old_value}</p><p class="text-gray-700 dark:text-gray-200"><strong>new_value:</strong> ${res.data.new_value}</p><p class="text-gray-700 dark:text-gray-200"><strong>ip_address:</strong> ${res.data.ip_address}</p><p class="text-gray-700 dark:text-gray-200"><strong>user_agent:</strong> ${res.data.user_agent}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View history', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/history.php', {id}, function(res){
        openModal('Edit history',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#edithistoryForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/history.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#historyModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#historyTable").load(location.href + " #historyTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/history.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#historyModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#historyTable").load(location.href + " #historyTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each history data
        $.post('<?= $path?>app/crud/api/history.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(history => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy history</h3>
                            <input type="hidden" name="ids[]" value="${history.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${history.id}]" value="${history.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="user_id">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${history.id}]" value="${history.user_id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="action">action</label>
                        <input type="action" id="action" name="action[${history.id}]" value="${history.action}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="entity_type">entity_type</label>
                        <input type="entity_type" id="entity_type" name="entity_type[${history.id}]" value="${history.entity_type}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="entity_id">entity_id</label>
                        <input type="entity_id" id="entity_id" name="entity_id[${history.id}]" value="${history.entity_id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="old_value">old_value</label>
                        <input type="old_value" id="old_value" name="old_value[${history.id}]" value="${history.old_value}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="new_value">new_value</label>
                        <input type="new_value" id="new_value" name="new_value[${history.id}]" value="${history.new_value}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="ip_address">ip_address</label>
                        <input type="ip_address" id="ip_address" name="ip_address[${history.id}]" value="${history.ip_address}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="user_agent">user_agent</label>
                        <input type="user_agent" id="user_agent" name="user_agent[${history.id}]" value="${history.user_agent}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${history.id}]" value="${history.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${history.id}]" value="${history.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${history.id}]" value="${history.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy history', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected history?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(history => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${history.id}</p><p>user_id: ${history.user_id}</p><p>action: ${history.action}</p><p>entity_type: ${history.entity_type}</p><p>entity_id: ${history.entity_id}</p><p>old_value: ${history.old_value}</p><p>new_value: ${history.new_value}</p><p>ip_address: ${history.ip_address}</p><p>user_agent: ${history.user_agent}</p><p>created_at: ${history.created_at}</p><p>updated_at: ${history.updated_at}</p><p>status: ${history.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/history.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each history's data
        $.post('<?= $path?>app/crud/api/history.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each history
                let html = '';
                res.data.forEach(history => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit history</h3>
                            <input type="hidden" name="ids[]" value="${history.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${history.id}]" value="${history.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="user_id" class="block text-gray-700 dark:text-gray-200">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${history.id}]" value="${history.user_id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="action" class="block text-gray-700 dark:text-gray-200">action</label>
                        <input type="action" id="action" name="action[${history.id}]" value="${history.action}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="entity_type" class="block text-gray-700 dark:text-gray-200">entity_type</label>
                        <input type="entity_type" id="entity_type" name="entity_type[${history.id}]" value="${history.entity_type}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="entity_id" class="block text-gray-700 dark:text-gray-200">entity_id</label>
                        <input type="entity_id" id="entity_id" name="entity_id[${history.id}]" value="${history.entity_id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="old_value" class="block text-gray-700 dark:text-gray-200">old_value</label>
                        <input type="old_value" id="old_value" name="old_value[${history.id}]" value="${history.old_value}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="new_value" class="block text-gray-700 dark:text-gray-200">new_value</label>
                        <input type="new_value" id="new_value" name="new_value[${history.id}]" value="${history.new_value}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="ip_address" class="block text-gray-700 dark:text-gray-200">ip_address</label>
                        <input type="ip_address" id="ip_address" name="ip_address[${history.id}]" value="${history.ip_address}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="user_agent" class="block text-gray-700 dark:text-gray-200">user_agent</label>
                        <input type="user_agent" id="user_agent" name="user_agent[${history.id}]" value="${history.user_agent}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${history.id}]" value="${history.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${history.id}]" value="${history.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${history.id}]" value="${history.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit history', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/history.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#historyModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#historyTable").load(location.href + " #historyTable");
...
// Collect all history fields
const excluded = ['id', 'user_id', 'action', 'entity_type', 'entity_id', 'old_value', 'new_value', 'ip_address', 'user_agent', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/history.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#historyModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#historyTable").load(location.href + " #historyTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/history.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#historyTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#historyTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
