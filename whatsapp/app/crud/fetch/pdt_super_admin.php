<?php
  $stmt = $pdo->query("SELECT * FROM pdt_super_admin");
  $pdt_super_admin = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
foreach ($pdt_super_admin as $item) {
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">pdt_super_admin</h1>
    ...
    <!-- Add pdt_super_admin Button -->
    <a href="#" id="addpdt_super_adminBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add pdt_super_admin
    </a>

    <!-- Reusable Modal -->
    <div id="pdt_super_adminModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">pdt_super_admin</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="pdt_super_adminTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">phoneNumber</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">created_at</th>
          ...
          <tbody>
            <?php $i=1; foreach($pdt_super_admin as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="pdt_super_adminChart" height="120"></canvas>
</div>

<!-- Grid Tab -->
<div id="tab-grid" class="hidden px-8 pt-4">
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    <?php foreach($pdt_super_admin as $item): ?>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          <!-- Optional pdt_super_admin Image -->
          <img
            src="<?= htmlspecialchars($item['image'] ?? 'https://placehold.co/600x400') ?>"
            alt="Image"
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="pdt_super_adminTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['phoneNumber']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/pdt_super_admin.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All pdt_super_admin</a>
</div>
...
const ctx = document.getElementById('pdt_super_adminChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'pdt_super_admin Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/pdt_super_admin.php', {
  ...
  function openModal(title,content){
    ...
    $('#pdt_super_adminModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add pdt_super_admin ---
  $('#addpdt_super_adminBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/pdt_super_admin.php', function(res){
      if(res){
        openModal('Add pdt_super_admin',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addpdt_super_adminForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php', $('#addpdt_super_adminForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
        window.location.reload();
        $('#addpdt_super_adminForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });

// --- View pdt_super_admin ---
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>fullName:</strong> ${res.data.fullName}</p><p class="text-gray-700 dark:text-gray-200"><strong>email:</strong> ${res.data.email}</p><p class="text-gray-700 dark:text-gray-200"><strong>phoneNumber:</strong> ${res.data.phoneNumber}</p><p class="text-gray-700 dark:text-gray-200"><strong>password:</strong> ${res.data.password}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View pdt_super_admin', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});


  // --- Edit pdt_super_admin ---
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/pdt_super_admin.php', {id}, function(res){
        openModal('Edit pdt_super_admin',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editpdt_super_adminForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
  ...
  // --- Delete pdt_super_admin ---
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this pdt_super_admin?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each pdt_super_admin data
        $.post('<?= $path?>app/crud/api/pdt_super_admin.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(pdt_super_admin => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy pdt_super_admin</h3>
                            <input type="hidden" name="ids[]" value="${pdt_super_admin.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${pdt_super_admin.id}]" value="${pdt_super_admin.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="fullName">fullName</label>
                        <input type="fullName" id="fullName" name="fullName[${pdt_super_admin.id}]" value="${pdt_super_admin.fullName}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${pdt_super_admin.id}]" value="${pdt_super_admin.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="phoneNumber">phoneNumber</label>
                        <input type="phoneNumber" id="phoneNumber" name="phoneNumber[${pdt_super_admin.id}]" value="${pdt_super_admin.phoneNumber}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="password">password</label>
                        <input type="password" id="password" name="password[${pdt_super_admin.id}]" value="${pdt_super_admin.password}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${pdt_super_admin.id}]" value="${pdt_super_admin.created_at}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy pdt_super_admin', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected pdt_super_admin?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(pdt_super_admin => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${pdt_super_admin.id}</p><p>fullName: ${pdt_super_admin.fullName}</p><p>email: ${pdt_super_admin.email}</p><p>phoneNumber: ${pdt_super_admin.phoneNumber}</p><p>password: ${pdt_super_admin.password}</p><p>created_at: ${pdt_super_admin.created_at}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/pdt_super_admin.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each pdt_super_admin's data
        $.post('<?= $path?>app/crud/api/pdt_super_admin.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each pdt_super_admin
                let html = '';
                res.data.forEach(pdt_super_admin => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit pdt_super_admin</h3>
                            <input type="hidden" name="ids[]" value="${pdt_super_admin.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${pdt_super_admin.id}]" value="${pdt_super_admin.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="fullName" class="block text-gray-700 dark:text-gray-200">fullName</label>
                        <input type="fullName" id="fullName" name="fullName[${pdt_super_admin.id}]" value="${pdt_super_admin.fullName}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${pdt_super_admin.id}]" value="${pdt_super_admin.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="phoneNumber" class="block text-gray-700 dark:text-gray-200">phoneNumber</label>
                        <input type="phoneNumber" id="phoneNumber" name="phoneNumber[${pdt_super_admin.id}]" value="${pdt_super_admin.phoneNumber}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="password" class="block text-gray-700 dark:text-gray-200">password</label>
                        <input type="password" id="password" name="password[${pdt_super_admin.id}]" value="${pdt_super_admin.password}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${pdt_super_admin.id}]" value="${pdt_super_admin.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit pdt_super_admin', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
...
// Collect all pdt_super_admin fields
const excluded = ['id', 'fullName', 'email', 'phoneNumber', 'password', 'created_at'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/pdt_super_admin.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#pdt_super_adminModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#pdt_super_adminTable").load(location.href + " #pdt_super_adminTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/pdt_super_admin.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#pdt_super_adminTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#pdt_super_adminTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#pdt_super_adminTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
