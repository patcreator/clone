...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">subscribers</h1>
    ...
    <a href="#" id="addsubscribersBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add subscribers
    </a>

    <!-- Reusable Modal -->
    <div id="subscribersModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">subscribers</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="subscribersTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($subscribers as $item): ?>
    ...
    <?php foreach($subscribers as $item): ?>
      ...
      >
        <img
          ...
          alt="subscribers Image"
          class="w-full h-40 object-cover"
        />
        <div class="p-4">
          <h3 class="text-lg font-bold text-blue-600"><?= htmlspecialchars($item['id']) ?></h3>
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['email']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['created_at']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="subscribersTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/subscribers.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All subscribers</a>
</div>
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'subscribers Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/subscribers.php', {
  ...
  function openModal(title,content){
    ...
    $('#subscribersModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add subscribers ---
  $('#addsubscribersBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/subscribers.php', function(res){
      if(res){
        openModal('Add subscribers',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addsubscribersForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/subscribers.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#subscribersTable").load(location.href + " #subscribersTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/subscribers.php', $('#addsubscribersForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#subscribersTable").load(location.href + " #subscribersTable");
        window.location.reload();
        $('#addsubscribersForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/subscribers.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/subscribers.php', {id}, function(res){
        openModal('Edit subscribers',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editsubscribersForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/subscribers.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#subscribersTable").load(location.href + " #subscribersTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/subscribers.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#subscribersTable").load(location.href + " #subscribersTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each subscribers data
        $.post('<?= $path?>app/crud/api/subscribers.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(subscribers => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy subscribers</h3>
                            <input type="hidden" name="ids[]" value="${subscribers.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${subscribers.id}]" value="${subscribers.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${subscribers.id}]" value="${subscribers.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${subscribers.id}]" value="${subscribers.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${subscribers.id}]" value="${subscribers.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${subscribers.id}]" value="${subscribers.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy subscribers', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected subscribers?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(subscribers => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${subscribers.id}</p><p>email: ${subscribers.email}</p><p>created_at: ${subscribers.created_at}</p><p>updated_at: ${subscribers.updated_at}</p><p>status: ${subscribers.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/subscribers.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each subscribers's data
        $.post('<?= $path?>app/crud/api/subscribers.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each subscribers
                let html = '';
                res.data.forEach(subscribers => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit subscribers</h3>
                            <input type="hidden" name="ids[]" value="${subscribers.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${subscribers.id}]" value="${subscribers.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${subscribers.id}]" value="${subscribers.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${subscribers.id}]" value="${subscribers.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${subscribers.id}]" value="${subscribers.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${subscribers.id}]" value="${subscribers.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit subscribers', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/subscribers.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#subscribersTable").load(location.href + " #subscribersTable");
...
// Collect all subscribers fields
const excluded = ['id', 'email', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/subscribers.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#subscribersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#subscribersTable").load(location.href + " #subscribersTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/subscribers.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#subscribersTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#subscribersTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
