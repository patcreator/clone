...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">help_topics</h1>
    ...
    <!-- Add help_topics Button -->
    <a href="#" id="addhelp_topicsBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add help_topics
    </a>

    <!-- Reusable Modal -->
    <div id="help_topicsModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">help_topics</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="help_topicsTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($help_topics as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="help_topicsChart" height="120"></canvas>
</div>

<!-- Grid Tab -->
<div id="tab-grid" class="hidden px-8 pt-4">
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    <?php foreach($help_topics as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['content']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['content']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="help_topicsTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/help_topics.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All help_topics</a>
</div>
...
const ctx = document.getElementById('help_topicsChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'help_topics Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/help_topics.php', {
  ...
  function openModal(title,content){
    ...
    $('#help_topicsModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add help_topics ---
  $('#addhelp_topicsBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/help_topics.php', function(res){
      if(res){
        openModal('Add help_topics',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addhelp_topicsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/help_topics.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_topicsTable").load(location.href + " #help_topicsTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/help_topics.php', $('#addhelp_topicsForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#help_topicsTable").load(location.href + " #help_topicsTable");
        window.location.reload();
        $('#addhelp_topicsForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/help_topics.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>title:</strong> ${res.data.title}</p><p class="text-gray-700 dark:text-gray-200"><strong>content:</strong> ${res.data.content}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View help_topics', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/help_topics.php', {id}, function(res){
        openModal('Edit help_topics',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#edithelp_topicsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/help_topics.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_topicsTable").load(location.href + " #help_topicsTable");
  ...
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this help_topics?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/help_topics.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_topicsTable").load(location.href + " #help_topicsTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each help_topics data
        $.post('<?= $path?>app/crud/api/help_topics.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(help_topics => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy help_topics</h3>
                            <input type="hidden" name="ids[]" value="${help_topics.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${help_topics.id}]" value="${help_topics.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="title">title</label>
                        <input type="title" id="title" name="title[${help_topics.id}]" value="${help_topics.title}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="content">content</label>
                        <input type="content" id="content" name="content[${help_topics.id}]" value="${help_topics.content}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${help_topics.id}]" value="${help_topics.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${help_topics.id}]" value="${help_topics.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${help_topics.id}]" value="${help_topics.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy help_topics', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected help_topics?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(help_topics => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${help_topics.id}</p><p>title: ${help_topics.title}</p><p>content: ${help_topics.content}</p><p>created_at: ${help_topics.created_at}</p><p>updated_at: ${help_topics.updated_at}</p><p>status: ${help_topics.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/help_topics.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each help_topics's data
        $.post('<?= $path?>app/crud/api/help_topics.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each help_topics
                let html = '';
                res.data.forEach(help_topics => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit help_topics</h3>
                            <input type="hidden" name="ids[]" value="${help_topics.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${help_topics.id}]" value="${help_topics.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="title" class="block text-gray-700 dark:text-gray-200">title</label>
                        <input type="title" id="title" name="title[${help_topics.id}]" value="${help_topics.title}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="content" class="block text-gray-700 dark:text-gray-200">content</label>
                        <input type="content" id="content" name="content[${help_topics.id}]" value="${help_topics.content}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${help_topics.id}]" value="${help_topics.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${help_topics.id}]" value="${help_topics.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${help_topics.id}]" value="${help_topics.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit help_topics', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/help_topics.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#help_topicsTable").load(location.href + " #help_topicsTable");
...
// Collect all help_topics fields
const excluded = ['id', 'title', 'content', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/help_topics.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#help_topicsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#help_topicsTable").load(location.href + " #help_topicsTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/help_topics.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#help_topicsTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#help_topicsTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#help_topicsTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
