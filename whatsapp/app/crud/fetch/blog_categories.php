...
$columns = ['blog_category_id','blog_category_name','blog_category_status','blog_category_slug','created_at'];
?>
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">blog_categories</h1>
    ...
    <!-- Add blog_categories Button -->
    <a href="#" id="addblog_categoriesBtn" class="mb-4 ml-8 px-4 py-2 bg-red-500 text-white rounded inline-block">
      Add blog_categories
    </a>

    <!-- Reusable Modal -->
    <div id="blog_categoriesModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">blog_categories</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="blog_categoriesTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
          ...
          <tbody>
            <?php $i=1; foreach($blog_categories as $item): ?>
            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 hover:dark:bg-gray-700">
                <td class="border-0 px-4 py-4 dark:bg-gray-800"><input type="checkbox" class="rowCheckbox" data-id="<?= $item['blog_category_id'] ?>"></td>

                <td class="border-0 px-4 py-4 dark:bg-gray-800"><?= $i++ ?></td>

                 
                    <td class="editable" data-id="<?= $item['blog_category_id'] ?>" data-field="blog_category_id" data-values=''><?= htmlspecialchars($item['blog_category_id']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['blog_category_id'] ?>" data-field="blog_category_name" data-values=''><?= htmlspecialchars($item['blog_category_name']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['blog_category_id'] ?>" data-field="blog_category_status" data-values=''><?= htmlspecialchars($item['blog_category_status']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['blog_category_id'] ?>" data-field="blog_category_slug" data-values=''><?= htmlspecialchars($item['blog_category_slug']) ?></td>
                    
                    <td class="editable" data-id="<?= $item['blog_category_id'] ?>" data-field="created_at" data-values=''><?= htmlspecialchars($item['created_at']) ?></td>
                    
                <td class="border-0 px-4 py-4 dark:bg-gray-800">
                  <button class="viewBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['blog_category_id'] ?>" title="view"><i class="mdi-24px mdi mdi-eye"></i></button>
                  <button class="editBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['blog_category_id'] ?>" title="edit"><i class="mdi-24px mdi mdi-pen"></i></button>
                  <button class="deleteBtn p-2 hover:bg-gray-100 rounded-full" data-id="<?= $item['blog_category_id'] ?>" title="delete"><i class="mdi-24px mdi mdi-delete"></i></button>
                </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="blog_categoriesChart" height="120"></canvas>
</div>
    ...
    <?php foreach($blog_categories as $key => $item): ?>
      ...
      >
        <img
          ...
          alt="blog_categories Image"
          class="w-full h-40 object-cover"
        />
        <div class="p-4">
          <h3 class="text-lg font-bold text-red-600"><?= htmlspecialchars($item['blog_category_id']) ?></h3>
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['blog_category_name']??'') ?></p>
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['blog_category_status']??'') ?></p>
          ...
          <div class="mt-4">
            ...
            >
              ...
              <p><?= htmlspecialchars($item['blog_category_slug'] ?? '') ?></p> <p><?= htmlspecialchars($item['created_at'] ?? '') ?></p> 
            </div>
          </div>
        </div>
      </div>
...
<div id="tab-list" class="hidden px-8 pt-4">
  <ul class="space-y-4">
    <?php foreach($blog_categories as $item): ?>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-red-500 transition">
        <div class="flex items-center space-x-4">
          <!-- Optional blog_categories Image -->
          <img
            src="<?= htmlspecialchars($item['image'] ?? 'https://placehold.co/600x400') ?>"
            alt="Image"
          ...
          <div>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['blog_category_id']) ?></p>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['blog_category_name']) ?></p>
            <p class="font-semibold text-red-600 dark:text-red-400"><?= htmlspecialchars($item['blog_category_status']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="blog_categoriesTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
          <th class="bg-gray-100 px-4 py-2 text-left dark:bg-gray-800">#</th>
         <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">blog_category_id</th> <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">blog_category_name</th> <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">blog_category_status</th> <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">blog_category_slug</th> <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">created_at</th> 
        </tr>
      </thead>
      <tbody>
        <?php $i = 1; foreach($blog_categories as $item): ?>
        <tr class="border-t">
          <td class="px-4 py-2 dark:bg-gray-800"><?= $i++ ?></td>
            <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['blog_category_slug']) ?></td><td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['created_at']) ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/blog_categories.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All blog_categories</a>
</div>
...
const ctx = document.getElementById('blog_categoriesChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'blog_categories Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/blog_categories.php', {
  ...
  function openModal(title,content){
    ...
    $('#blog_categoriesModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add blog_categories ---
  $('#addblog_categoriesBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/blog_categories.php', function(res){
      if(res){
        openModal('Add blog_categories',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addblog_categoriesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/blog_categories.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/blog_categories.php', $('#addblog_categoriesForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
        window.location.reload();
        $('#addblog_categoriesForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/blog_categories.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>blog_category_id:</strong> ${res.data.blog_category_id}</p> <p class="text-gray-700 dark:text-gray-200"><strong>blog_category_name:</strong> ${res.data.blog_category_name}</p> <p class="text-gray-700 dark:text-gray-200"><strong>blog_category_status:</strong> ${res.data.blog_category_status}</p> <p class="text-gray-700 dark:text-gray-200"><strong>blog_category_slug:</strong> ${res.data.blog_category_slug}</p> <p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p> 
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-red-500 text-white px-6 py-2 rounded hover:bg-red-600">Close</button>
          </div>
        `;
        openModal('View blog_categories', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});


  // --- Edit blog_categories ---
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/blog_categories.php', {id}, function(res){
        openModal('Edit blog_categories',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editblog_categoriesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/blog_categories.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
  ...
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this blog_categories?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/blog_categories.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each blog_categories data
        $.post('<?= $path?>app/crud/api/blog_categories.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(blog_categories => {
                    html += `
                    <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                        <h3 class="font-bold text-lg mb-2 text-green-500">Copy blog_categories</h3>
                        <input type="hidden" name="ids[]" value="${blog_categories.id}">
                        <div class="grid grid-cols-2 gap-4">
                        
                            <div>
                                <label for="blog_category_id">blog_category_id</label>
                                <input type="text" id="blog_category_id" name="blog_category_id[${blog_categories.id}]" value="${blog_categories.blog_category_id}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="blog_category_name">blog_category_name</label>
                                <input type="text" id="blog_category_name" name="blog_category_name[${blog_categories.id}]" value="${blog_categories.blog_category_name}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="blog_category_status">blog_category_status</label>
                                <input type="text" id="blog_category_status" name="blog_category_status[${blog_categories.id}]" value="${blog_categories.blog_category_status}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="blog_category_slug">blog_category_slug</label>
                                <input type="text" id="blog_category_slug" name="blog_category_slug[${blog_categories.id}]" value="${blog_categories.blog_category_slug}" class="w-full p-2 border rounded mb-2">
                            </div> 
                            <div>
                                <label for="created_at">created_at</label>
                                <input type="text" id="created_at" name="created_at[${blog_categories.id}]" value="${blog_categories.created_at}" class="w-full p-2 border rounded mb-2">
                            </div> 
                        </div>
                    </div>
                    `;
                ...
                openModal('Bulk Copy blog_categories', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected blog_categories?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(blog_categories => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                        ...
                                        <p>blog_category_id: ${blog_categories.blog_category_id}</p>
                                    
                                        <p>blog_category_name: ${blog_categories.blog_category_name}</p>
                                    
                                        <p>blog_category_status: ${blog_categories.blog_category_status}</p>
                                    
                                        <p>blog_category_slug: ${blog_categories.blog_category_slug}</p>
                                    
                                        <p>created_at: ${blog_categories.created_at}</p>
          ...
          window.location.href = `<?= $path?>app/crud/api/blog_categories.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each blog_categories's data
        $.post('<?= $path?>app/crud/api/blog_categories.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each blog_categories
                let html = '';
                res.data.forEach(blog_categories => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-red-500">Edit blog_categories</h3>
                            <input type="hidden" name="ids[]" value="${blog_categories.id}">
                            <div class="grid grid-cols-2 gap-4">

                                
                                        <div>
                                            <label for="blog_category_slug" class="block text-gray-700 dark:text-gray-200">blog_category_slug</label>
                                            <input type="text" id="blog_category_slug" name="blog_category_slug[${blog_categories.id}]" value="${blog_categories.blog_category_slug}" class="w-full p-2 border rounded mb-2" required>
                                        </div> 
                                    
                                        <div>
                                            <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                                            <input type="text" id="created_at" name="created_at[${blog_categories.id}]" value="${blog_categories.created_at}" class="w-full p-2 border rounded mb-2" required>
                                        </div> 
                ...
                openModal('Bulk Edit blog_categories', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/blog_categories.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
...
// Collect all blog_categories fields
const excluded = [
    'blog_category_id',
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/blog_categories.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#blog_categoriesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#blog_categoriesTable").load(location.href + " #blog_categoriesTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/blog_categories.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#blog_categoriesTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#blog_categoriesTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#blog_categoriesTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
