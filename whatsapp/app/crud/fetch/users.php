...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">users</h1>
    ...
    <!-- Add users Button -->
    <a href="#" id="addusersBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add users
    </a>

    <!-- Reusable Modal -->
    <div id="usersModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">users</h2>
        <span></span>
      </div>
    ...
    <?php foreach($users as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['username']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['username']) ?></p>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/users.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All users</a>
</div>
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/users.php', {
  ...
  function openModal(title,content){
    ...
    $('#usersModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#usersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });
  ...
  $('#addusersBtn').click(function(e){
    ...
    $.post('<?= $path?>app/crud/forms/create/users.php', function(res){
      if(res){
        openModal('Add users',res);
  ...
  $(document).on('submit','#addusersForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/users.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/users.php', $('#addusersForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#usersTable").load(location.href + " #usersTable");
        window.location.reload();
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/users.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>username:</strong> ${res.data.username}</p><p class="text-gray-700 dark:text-gray-200"><strong>email:</strong> ${res.data.email}</p><p class="text-gray-700 dark:text-gray-200"><strong>password:</strong> ${res.data.password}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/users.php', {id}, function(res){
        openModal('Edit users',res);
  ...
  $(document).on('submit','#editusersForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/users.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#usersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#usersTable").load(location.href + " #usersTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/users.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#usersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#usersTable").load(location.href + " #usersTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each users data
        $.post('<?= $path?>app/crud/api/users.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(users => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy users</h3>
                            <input type="hidden" name="ids[]" value="${users.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${users.id}]" value="${users.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="username">username</label>
                        <input type="username" id="username" name="username[${users.id}]" value="${users.username}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="email">email</label>
                        <input type="email" id="email" name="email[${users.id}]" value="${users.email}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="password">password</label>
                        <input type="password" id="password" name="password[${users.id}]" value="${users.password}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${users.id}]" value="${users.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${users.id}]" value="${users.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${users.id}]" value="${users.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(users => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${users.id}</p><p>username: ${users.username}</p><p>email: ${users.email}</p><p>password: ${users.password}</p><p>created_at: ${users.created_at}</p><p>updated_at: ${users.updated_at}</p><p>status: ${users.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/users.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each users's data
        $.post('<?= $path?>app/crud/api/users.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each users
                let html = '';
                res.data.forEach(users => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit users</h3>
                            <input type="hidden" name="ids[]" value="${users.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${users.id}]" value="${users.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="username" class="block text-gray-700 dark:text-gray-200">username</label>
                        <input type="username" id="username" name="username[${users.id}]" value="${users.username}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="email" class="block text-gray-700 dark:text-gray-200">email</label>
                        <input type="email" id="email" name="email[${users.id}]" value="${users.email}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="password" class="block text-gray-700 dark:text-gray-200">password</label>
                        <input type="password" id="password" name="password[${users.id}]" value="${users.password}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${users.id}]" value="${users.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${users.id}]" value="${users.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${users.id}]" value="${users.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/users.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#usersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#usersTable").load(location.href + " #usersTable");
...
// Collect all users fields
const excluded = ['id', 'username', 'email', 'password', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/users.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#usersModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#usersTable").load(location.href + " #usersTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/users.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
