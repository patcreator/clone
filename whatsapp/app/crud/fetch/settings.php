...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">settings</h1>
    ...
    <!-- Add settings Button -->
    <a href="#" id="addsettingsBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add settings
    </a>

    <!-- Reusable Modal -->
    <div id="settingsModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">settings</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="settingsTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">value</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($settings as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="settingsChart" height="120"></canvas>
</div>
    ...
    <?php foreach($settings as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-gray-600 dark:text-gray-300"><?= htmlspecialchars($item['name']??'') ?></p>
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['value']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['value']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="settingsTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['value']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/settings.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All settings</a>
</div>
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'settings Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/settings.php', {
  ...
  function openModal(title,content){
    ...
    $('#settingsModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add settings ---
  $('#addsettingsBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/settings.php', function(res){
      if(res){
        openModal('Add settings',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addsettingsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/settings.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#settingsTable").load(location.href + " #settingsTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/settings.php', $('#addsettingsForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#settingsTable").load(location.href + " #settingsTable");
        window.location.reload();
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/settings.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>name:</strong> ${res.data.name}</p><p class="text-gray-700 dark:text-gray-200"><strong>value:</strong> ${res.data.value}</p><p class="text-gray-700 dark:text-gray-200"><strong>type:</strong> ${res.data.type}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View settings', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/settings.php', {id}, function(res){
        openModal('Edit settings',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#editsettingsForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/settings.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#settingsTable").load(location.href + " #settingsTable");
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/settings.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#settingsTable").load(location.href + " #settingsTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each settings data
        $.post('<?= $path?>app/crud/api/settings.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(settings => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy settings</h3>
                            <input type="hidden" name="ids[]" value="${settings.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${settings.id}]" value="${settings.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="name">name</label>
                        <input type="name" id="name" name="name[${settings.id}]" value="${settings.name}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="value">value</label>
                        <input type="value" id="value" name="value[${settings.id}]" value="${settings.value}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="type">type</label>
                        <input type="type" id="type" name="type[${settings.id}]" value="${settings.type}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${settings.id}]" value="${settings.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${settings.id}]" value="${settings.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${settings.id}]" value="${settings.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy settings', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected settings?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(settings => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${settings.id}</p><p>name: ${settings.name}</p><p>value: ${settings.value}</p><p>type: ${settings.type}</p><p>created_at: ${settings.created_at}</p><p>updated_at: ${settings.updated_at}</p><p>status: ${settings.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/settings.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each settings's data
        $.post('<?= $path?>app/crud/api/settings.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each settings
                let html = '';
                res.data.forEach(settings => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit settings</h3>
                            <input type="hidden" name="ids[]" value="${settings.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${settings.id}]" value="${settings.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="name" class="block text-gray-700 dark:text-gray-200">name</label>
                        <input type="name" id="name" name="name[${settings.id}]" value="${settings.name}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="value" class="block text-gray-700 dark:text-gray-200">value</label>
                        <input type="value" id="value" name="value[${settings.id}]" value="${settings.value}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="type" class="block text-gray-700 dark:text-gray-200">type</label>
                        <input type="type" id="type" name="type[${settings.id}]" value="${settings.type}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${settings.id}]" value="${settings.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${settings.id}]" value="${settings.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${settings.id}]" value="${settings.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit settings', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/settings.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#settingsTable").load(location.href + " #settingsTable");
...
const excluded = ['id', 'name', 'value', 'type', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/settings.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#settingsModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#settingsTable").load(location.href + " #settingsTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/settings.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#settingsTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#settingsTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
