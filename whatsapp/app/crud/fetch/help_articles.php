<?php
  ...
  $help_articles = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">help_articles</h1>
    ...
    <!-- Add help_articles Button -->
    <a href="#" id="addhelp_articlesBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add help_articles
    </a>

    <!-- Reusable Modal -->
    <div id="help_articlesModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">help_articles</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="help_articlesTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">category</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">is_active</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($help_articles as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="help_articlesChart" height="120"></canvas>
</div>

<!-- Grid Tab -->
<div id="tab-grid" class="hidden px-8 pt-4">
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    <?php foreach($help_articles as $item): ?>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['title']) ?></p>
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['slug']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="help_articlesTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['category']) ?></td>
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['is_active']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/help_articles.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All help_articles</a>
</div>
...
const ctx = document.getElementById('help_articlesChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'help_articles Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/help_articles.php', {
  ...
  function openModal(title,content){
    ...
    $('#help_articlesModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add help_articles ---
  $('#addhelp_articlesBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/help_articles.php', function(res){
      if(res){
        openModal('Add help_articles',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#addhelp_articlesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/help_articles.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_articlesTable").load(location.href + " #help_articlesTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/help_articles.php', $('#addhelp_articlesForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#help_articlesTable").load(location.href + " #help_articlesTable");
        window.location.reload();
        $('#addhelp_articlesForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/help_articles.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>title:</strong> ${res.data.title}</p><p class="text-gray-700 dark:text-gray-200"><strong>slug:</strong> ${res.data.slug}</p><p class="text-gray-700 dark:text-gray-200"><strong>category:</strong> ${res.data.category}</p><p class="text-gray-700 dark:text-gray-200"><strong>content:</strong> ${res.data.content}</p><p class="text-gray-700 dark:text-gray-200"><strong>keywords:</strong> ${res.data.keywords}</p><p class="text-gray-700 dark:text-gray-200"><strong>is_active:</strong> ${res.data.is_active}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View help_articles', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/help_articles.php', {id}, function(res){
        openModal('Edit help_articles',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#edithelp_articlesForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/help_articles.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_articlesTable").load(location.href + " #help_articlesTable");
  ...
  // --- Delete help_articles ---
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this help_articles?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/help_articles.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#help_articlesTable").load(location.href + " #help_articlesTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each help_articles data
        $.post('<?= $path?>app/crud/api/help_articles.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(help_articles => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy help_articles</h3>
                            <input type="hidden" name="ids[]" value="${help_articles.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${help_articles.id}]" value="${help_articles.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="title">title</label>
                        <input type="title" id="title" name="title[${help_articles.id}]" value="${help_articles.title}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="slug">slug</label>
                        <input type="slug" id="slug" name="slug[${help_articles.id}]" value="${help_articles.slug}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="category">category</label>
                        <input type="category" id="category" name="category[${help_articles.id}]" value="${help_articles.category}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="content">content</label>
                        <input type="content" id="content" name="content[${help_articles.id}]" value="${help_articles.content}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="keywords">keywords</label>
                        <input type="keywords" id="keywords" name="keywords[${help_articles.id}]" value="${help_articles.keywords}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="is_active">is_active</label>
                        <input type="is_active" id="is_active" name="is_active[${help_articles.id}]" value="${help_articles.is_active}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${help_articles.id}]" value="${help_articles.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${help_articles.id}]" value="${help_articles.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${help_articles.id}]" value="${help_articles.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy help_articles', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected help_articles?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(help_articles => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${help_articles.id}</p><p>title: ${help_articles.title}</p><p>slug: ${help_articles.slug}</p><p>category: ${help_articles.category}</p><p>content: ${help_articles.content}</p><p>keywords: ${help_articles.keywords}</p><p>is_active: ${help_articles.is_active}</p><p>created_at: ${help_articles.created_at}</p><p>updated_at: ${help_articles.updated_at}</p><p>status: ${help_articles.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/help_articles.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each help_articles's data
        $.post('<?= $path?>app/crud/api/help_articles.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each help_articles
                let html = '';
                res.data.forEach(help_articles => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit help_articles</h3>
                            <input type="hidden" name="ids[]" value="${help_articles.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${help_articles.id}]" value="${help_articles.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="title" class="block text-gray-700 dark:text-gray-200">title</label>
                        <input type="title" id="title" name="title[${help_articles.id}]" value="${help_articles.title}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="slug" class="block text-gray-700 dark:text-gray-200">slug</label>
                        <input type="slug" id="slug" name="slug[${help_articles.id}]" value="${help_articles.slug}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="category" class="block text-gray-700 dark:text-gray-200">category</label>
                        <input type="category" id="category" name="category[${help_articles.id}]" value="${help_articles.category}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="content" class="block text-gray-700 dark:text-gray-200">content</label>
                        <input type="content" id="content" name="content[${help_articles.id}]" value="${help_articles.content}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="keywords" class="block text-gray-700 dark:text-gray-200">keywords</label>
                        <input type="keywords" id="keywords" name="keywords[${help_articles.id}]" value="${help_articles.keywords}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="is_active" class="block text-gray-700 dark:text-gray-200">is_active</label>
                        <input type="is_active" id="is_active" name="is_active[${help_articles.id}]" value="${help_articles.is_active}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${help_articles.id}]" value="${help_articles.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${help_articles.id}]" value="${help_articles.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${help_articles.id}]" value="${help_articles.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit help_articles', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/help_articles.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#help_articlesTable").load(location.href + " #help_articlesTable");
...
// Collect all help_articles fields
const excluded = ['id', 'title', 'slug', 'category', 'content', 'keywords', 'is_active', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/help_articles.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#help_articlesModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#help_articlesTable").load(location.href + " #help_articlesTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/help_articles.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#help_articlesTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#help_articlesTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#help_articlesTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
