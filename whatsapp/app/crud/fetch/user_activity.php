<?php
  $stmt = $pdo->query("SELECT * FROM user_activity");
...
<script src="https://cdn.datatables.net/2.1.7/js/dataTables.min.js"></script>
    ...
    <h1 class="text-2xl font-bold mb-6 px-8">user_activity</h1>
    ...
    <!-- Add user_activity Button -->
    <a href="#" id="adduser_activityBtn" class="mb-4 ml-8 px-4 py-2 bg-blue-500 text-white rounded inline-block">
      Add user_activity
    </a>

    <!-- Reusable Modal -->
    <div id="user_activityModal" class="fixed inset-0 w-full h-full bg-white dark:bg-gray-900 dark:text-white z-50 overflow-y-auto hidden">
      <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-xl font-bold" id="modalTitle">user_activity</h2>
        <span></span>
      </div>
    ...
    <div class="overflow-x-auto px-8 pt-2">
      <div id="tab-tables">
        <table id="user_activityTable" class="display min-w-full bg-white rounded shadow w-full text-sm text-left text-gray-700 dark:text-gray-200">
          <thead>
            <tr>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">activity_type</th>
        ...
        <th class="border-0 bg-gray-100 px-4 py-4 dark:bg-gray-800 text-left bg-gray-100 dark:bg-gray-800 font-semibold text-gray-700 dark:text-gray-300 px-4 py-3">status</th>
          ...
          <tbody>
            <?php $i=1; foreach($user_activity as $item): ?>
...
<div id="tab-charts" class="hidden px-8 pt-4">
  <canvas id="user_activityChart" height="120"></canvas>
</div>
    ...
    <?php foreach($user_activity as $item): ?>
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Status: <?= htmlspecialchars($item['activity_type']??'') ?></p>
...
<li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition">
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="font-semibold text-blue-600 dark:text-blue-400"><?= htmlspecialchars($item['activity_type']) ?></p>
...
<div id="tab-print" class="hidden px-8 pt-4 print:block">
  ...
  <div class="overflow-auto">
    <table id="user_activityTable" class="min-w-full bg-white rounded shadow text-sm">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr class="border-t">
          ...
          <td class="px-4 py-2 dark:bg-gray-800"><?= htmlspecialchars($item['activity_type']) ?></td>
...
<div id="tab-export" class="hidden px-8 pt-4 h-1/3">
  <a href="<?= $path?>app/crud/api/user_activity.php?action=export" class="bg-green-500 text-white px-4 py-2 rounded">Export All user_activity</a>
</div>
...
const ctx = document.getElementById('user_activityChart');
if (ctx) {
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: 'user_activity Count' }
        }
      }
    }
  });
...
$(document).on('click', 'td.editable', function () {
  ...
  input.on('blur keydown', function (e) {
    if (e.type === 'blur' || (e.type === 'keydown' && e.key === 'Enter')) {
      ...
      $.post('<?= $path?>app/crud/api/user_activity.php', {
  ...
  function openModal(title,content){
    ...
    $('#user_activityModal').hide().removeClass('hidden').fadeIn(200);
  }

  $(document).on('click','.modal-close', function(){
    $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
  });

  // --- Add user_activity ---
  $('#adduser_activityBtn').click(function(e){
    e.preventDefault();
      
    $.post('<?= $path?>app/crud/forms/create/user_activity.php', function(res){
      if(res){
        openModal('Add user_activity',res);
  ...
  // --- Submit Add ---
  $(document).on('submit','#adduser_activityForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/user_activity.php', $(this).serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#user_activityTable").load(location.href + " #user_activityTable");
  ...
  $(document).on('click','#saveContinueBtn', function(){
    $.post('<?= $path?>app/crud/api/user_activity.php', $('#adduser_activityForm').serialize()+'&action=insert', function(res){
      if(res.success){
        showToast(res.message);
        // $("#user_activityTable").load(location.href + " #user_activityTable");
        window.location.reload();
        $('#adduser_activityForm')[0].reset();
      } else {showToast(res.message,'error');}
    },'json').fail(()=>showToast('Server error','error'));
  });
...
$(document).on('click','.viewBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/user_activity.php',{action:'view',id}, function(res){
      if(res.success){
        let html = `
          ...
          <div class="grid grid-cols-2 gap-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
            <p class="text-gray-700 dark:text-gray-200"><strong>id:</strong> ${res.data.id}</p><p class="text-gray-700 dark:text-gray-200"><strong>user_id:</strong> ${res.data.user_id}</p><p class="text-gray-700 dark:text-gray-200"><strong>activity_type:</strong> ${res.data.activity_type}</p><p class="text-gray-700 dark:text-gray-200"><strong>description:</strong> ${res.data.description}</p><p class="text-gray-700 dark:text-gray-200"><strong>ip_address:</strong> ${res.data.ip_address}</p><p class="text-gray-700 dark:text-gray-200"><strong>created_at:</strong> ${res.data.created_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>updated_at:</strong> ${res.data.updated_at}</p><p class="text-gray-700 dark:text-gray-200"><strong>status:</strong> ${res.data.status}</p>
          <div class="flex justify-end mt-4">
            <button type="button" class="modal-close bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Close</button>
          </div>
        `;
        openModal('View user_activity', html);
      } else showToast(res.message,'error');
    },'json').fail(()=>showToast('Server error','error'));
});
  ...
  $(document).on('click','.editBtn', function(){
    let id=$(this).data('id');
    $.get('<?= $path?>app/crud/forms/update/user_activity.php', {id}, function(res){
        openModal('Edit user_activity',res);
    }).fail(()=>showToast('Server error','error'));
  });


  // --- Submit Edit ---
  $(document).on('submit','#edituser_activityForm', function(e){
    e.preventDefault();
    $.post('<?= $path?>app/crud/api/user_activity.php', $(this).serialize()+'&action=edit', function(res){
      if(res.success){
        showToast(res.message);
        $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#user_activityTable").load(location.href + " #user_activityTable");
  ...
  $(document).on('click','.deleteBtn', function(){
    let id=$(this).data('id');
    let html=`
      <p>Are you sure you want to delete this user_activity?</p>
  ...
  $(document).on('click','#confirmDeleteBtn', function(){
    let id=$(this).data('id');
    $.post('<?= $path?>app/crud/api/user_activity.php',{action:'delete',id}, function(res){
      if(res.success){
        showToast(res.message);
        $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
        // $("#user_activityTable").load(location.href + " #user_activityTable");
...
$('#applyBulkAction').on('click', function(){
    ...
    if(action === 'copy'){
        // Bulk Copy: fetch each user_activity data
        $.post('<?= $path?>app/crud/api/user_activity.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                let html = '<form id="bulkCopyForm">';
                res.data.forEach(user_activity => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-green-500">Copy user_activity</h3>
                            <input type="hidden" name="ids[]" value="${user_activity.id}">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                        <label for="id">id</label>
                        <input type="id" id="id" name="id[${user_activity.id}]" value="${user_activity.id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="user_id">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${user_activity.id}]" value="${user_activity.user_id}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="activity_type">activity_type</label>
                        <input type="activity_type" id="activity_type" name="activity_type[${user_activity.id}]" value="${user_activity.activity_type}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="description">description</label>
                        <input type="description" id="description" name="description[${user_activity.id}]" value="${user_activity.description}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="ip_address">ip_address</label>
                        <input type="ip_address" id="ip_address" name="ip_address[${user_activity.id}]" value="${user_activity.ip_address}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="created_at">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${user_activity.id}]" value="${user_activity.created_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="updated_at">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${user_activity.id}]" value="${user_activity.updated_at}" class="w-full p-2 border rounded mb-2">
                    </div><div>
                        <label for="status">status</label>
                        <input type="status" id="status" name="status[${user_activity.id}]" value="${user_activity.status}" class="w-full p-2 border rounded mb-2">
                    </div>
                            </div>
                        </div>
                    `;
                ...
                openModal('Bulk Copy user_activity', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    }
        ...
        let html = `<p>Are you sure you want to delete selected user_activity?</p>
                    <div class="flex justify-end gap-3 mt-4">
        ...
        openModal('Confirm Bulk Delete', html);
              ...
              if(res.success){
                  ...
                  res.data.forEach(user_activity => {
                      html += `<div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                                  <p>id: ${user_activity.id}</p><p>user_id: ${user_activity.user_id}</p><p>activity_type: ${user_activity.activity_type}</p><p>description: ${user_activity.description}</p><p>ip_address: ${user_activity.ip_address}</p><p>created_at: ${user_activity.created_at}</p><p>updated_at: ${user_activity.updated_at}</p><p>status: ${user_activity.status}</p>
                                </div>`;
                  });
          ...
          window.location.href = `<?= $path?>app/crud/api/user_activity.php?action=bulk_export&ids=${idsParam}`;
      }


    else if(action === 'edit'){
        // Bulk Edit: fetch each user_activity's data
        $.post('<?= $path?>app/crud/api/user_activity.php',{action:'bulk_view', ids: ids}, function(res){
            if(res.success){
                // Create forms for each user_activity
                let html = '';
                res.data.forEach(user_activity => {
                    html += `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded shadow mb-4">
                            <h3 class="font-bold text-lg mb-2 text-blue-500">Edit user_activity</h3>
                            <input type="hidden" name="ids[]" value="${user_activity.id}">
                            <div class="grid grid-cols-2 gap-4">
                             <div>
                        <label for="id" class="block text-gray-700 dark:text-gray-200">id</label>
                        <input type="id" id="id" name="id[${user_activity.id}]" value="${user_activity.id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="user_id" class="block text-gray-700 dark:text-gray-200">user_id</label>
                        <input type="user_id" id="user_id" name="user_id[${user_activity.id}]" value="${user_activity.user_id}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="activity_type" class="block text-gray-700 dark:text-gray-200">activity_type</label>
                        <input type="activity_type" id="activity_type" name="activity_type[${user_activity.id}]" value="${user_activity.activity_type}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="description" class="block text-gray-700 dark:text-gray-200">description</label>
                        <input type="description" id="description" name="description[${user_activity.id}]" value="${user_activity.description}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="ip_address" class="block text-gray-700 dark:text-gray-200">ip_address</label>
                        <input type="ip_address" id="ip_address" name="ip_address[${user_activity.id}]" value="${user_activity.ip_address}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="created_at" class="block text-gray-700 dark:text-gray-200">created_at</label>
                        <input type="created_at" id="created_at" name="created_at[${user_activity.id}]" value="${user_activity.created_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="updated_at" class="block text-gray-700 dark:text-gray-200">updated_at</label>
                        <input type="updated_at" id="updated_at" name="updated_at[${user_activity.id}]" value="${user_activity.updated_at}" class="w-full p-2 border rounded mb-2" required>
                      </div>
        <div>
                        <label for="status" class="block text-gray-700 dark:text-gray-200">status</label>
                        <input type="status" id="status" name="status[${user_activity.id}]" value="${user_activity.status}" class="w-full p-2 border rounded mb-2" required>
                      </div>
                ...
                openModal('Bulk Edit user_activity', html);
            } else showToast(res.message,'error');
        },'json').fail(()=>showToast('Server error','error'));
    } else {
...
$(document).on('click','#bulkDeleteBtn', function(){
    ...
    $.post('<?= $path?>app/crud/api/user_activity.php',{action:'bulk_delete',ids:ids}, function(res){
        if(res.success){
            showToast(res.message);
            $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#user_activityTable").load(location.href + " #user_activityTable");
...
// Collect all user_activity fields
const excluded = ['id', 'user_id', 'activity_type', 'description', 'ip_address', 'created_at', 'updated_at', 'status'];
...
$('#modalContent').find('input, select').each(function() {
    ...
    $.post('<?= $path?>app/crud/api/user_activity.php', formData, function(res){
        if(res.success){
            showToast(res.message);
            $('#user_activityModal').fadeOut(200,function(){ $(this).addClass('hidden'); });
            // $("#user_activityTable").load(location.href + " #user_activityTable");
...
$('#importForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= $path?>app/crud/api/user_activity.php',
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function(){
  ...
  let table = new DataTable('#user_activityTable', {
    pageLength: 5,
  ...
  function applyDarkMode(){
    if($('html').hasClass('dark')){
      $('#user_activityTable').addClass('stripe hover text-gray-200 bg-gray-900');
    } else {
      $('#user_activityTable').removeClass('text-gray-200 bg-gray-900');
    }
  }
