...
function getCount($pdo, $period) {
    switch($period) {
        case 'today':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE DATE(created_at) = CURDATE()");
            break;
        case 'yesterday':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE DATE(created_at) = CURDATE() - INTERVAL 1 DAY");
            break;
        case 'week':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE YEARWEEK(created_at,1) = YEARWEEK(CURDATE(),1)");
            break;
        case 'last_week':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE YEARWEEK(created_at,1) = YEARWEEK(CURDATE(),1) - 1");
            break;
        case 'month':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())");
            break;
        case 'last_month':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE MONTH(created_at) = MONTH(CURDATE() - INTERVAL 1 MONTH) AND YEAR(created_at) = YEAR(CURDATE() - INTERVAL 1 MONTH)");
            break;
        case 'year':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE YEAR(created_at) = YEAR(CURDATE())");
            break;
        case 'last_year':
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog WHERE YEAR(created_at) = YEAR(CURDATE()) - 1");
            break;
        case 'all':
        default:
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM blog");
            break;
    }
...
<div class="p-5 mt-9">
    <div class="container mx-auto px-4">
        <h1 class="text-3xl font-bold mb-8 text-gray-800 dark:text-gray-300">blog  Registration Dashboard</h1>
        

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
    <!-- Today -->
    <div class="dark:bg-gray-800 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-semibold dark:text-gray-300 text-gray-700 mb-2">blog Registered Today</h2>
    ...
    <!-- This Week -->
    <div class="dark:bg-gray-800 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-semibold dark:text-gray-300 text-gray-700 mb-2">blog This Week</h2>
    ...
    <!-- This Month -->
    <div class="dark:bg-gray-800 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-semibold dark:text-gray-300 text-gray-700 mb-2">blog This Month</h2>
    ...
    <!-- This Year -->
    <div class="dark:bg-gray-800 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-semibold dark:text-gray-300 text-gray-700 mb-2">blog This Year</h2>
    ...
    <!-- All blog -->
    <div class="dark:bg-gray-800 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-semibold dark:text-gray-300 text-gray-700 mb-2">All blog (Total)</h2>
        <p class="text-3xl font-bold text-indigo-600"><?= number_format($all_blog) ?></p>
        ...
        <!-- Dynamic Chart: blog by Selected Period (Line Chart) -->
        ...
        <div class="flex justify-start mb-8">
            ...
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-300 mb-4" id="chartTitle">blog Registered by Period</h2>
    ...
    fetch(`<?= $path?>app/crud/api/dashboard_blog.php?period=${period}`)
        .then(res => res.json())
        .then(data => {
            ...
            currentBreakdownChart = new Chart(pieCtx, {
                type: 'pie',
                data: {
                    labels: period === 'all' ? ['Total blog', 'Inactive/Other'] : ['This Period','Prior Periods'],
                    datasets: [{
                        data: data.data,
            ...
            document.getElementById('pieTitle').textContent = period === 'all' ? 'All blog Breakdown' : `Registration Breakdown (This ${period} vs. Prior)`;
        });
}
        ...
        function updateBreakdownChart(period) {
            ...
            if (period === 'all') {
                labels = ['Total blog', 'Inactive/Other'];
            ...
            currentBreakdownChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: period === 'all' ? 'All blog Total' : `Breakdown for ${period.charAt(0).toUpperCase() + period.slice(1)}`
                        },
                        tooltip: {
                            callbacks: {
