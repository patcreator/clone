...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM history WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($history[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            $history = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'history_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($history[0]));
            foreach($history as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'user_id', 'action', 'entity_type', 'entity_id', 'old_value', 'new_value', 'ip_address', 'user_agent', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO history (id, user_id, action, entity_type, entity_id, old_value, new_value, ip_address, user_agent, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'history added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert history");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `history` SET  
                   id=?, user_id=?, action=?, entity_type=?, entity_id=?, old_value=?, new_value=?, ip_address=?, user_agent=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status, $id])) {
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM history WHERE id=?");
            $stmt->execute([$id]);
            $history = $stmt->fetch(PDO::FETCH_ASSOC);
            if($history) $response = ['success'=>true, 'data'=>$history];
            else throw new Exception("history not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM history WHERE id IN ($in)");
            $stmt->execute($ids);
            $history = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `history` SET  
                   id=?, user_id=?, action=?, entity_type=?, entity_id=?, old_value=?, new_value=?, ip_address=?, user_agent=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO history (id, user_id, action, entity_type, entity_id, old_value, new_value, ip_address, user_agent, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'user_id', 'action', 'entity_type', 'entity_id', 'old_value', 'new_value', 'ip_address', 'user_agent', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO history (id, user_id, action, entity_type, entity_id, old_value, new_value, ip_address, user_agent, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM history WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' history deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM history WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $history = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($history as $u){
                $stmt2 = $pdo->prepare("INSERT INTO history (id, user_id, action, entity_type, entity_id, old_value, new_value, ip_address, user_agent, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($history).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO history ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE history SET id=?, user_id=?, action=?, entity_type=?, entity_id=?, old_value=?, new_value=?, ip_address=?, user_agent=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $user_id, $action, $entity_type, $entity_id, $old_value, $new_value, $ip_address, $user_agent, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in history'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'user_id', 'action', 'entity_type', 'entity_id', 'old_value', 'new_value', 'ip_address', 'user_agent', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('history',[$field=>$value]);

            $unique_cols = unique_cols('history'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE history SET `$field` = :value WHERE id = :id");
