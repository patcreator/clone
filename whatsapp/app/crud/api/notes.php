...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM notes WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($notes[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            $notes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'notes_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($notes[0]));
            foreach($notes as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("INSERT INTO notes (id, title, content, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $title, $content, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'notes added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert notes");
            }
            break;
        ...
        case 'view':
            ...
            if($notes) $response = ['success'=>true, 'data'=>$notes];
            else throw new Exception("notes not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM notes WHERE id IN ($in)");
            $stmt->execute($ids);
            $notes = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_delete':
            ...
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' notes deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM notes WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $notes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($notes as $u){
                $stmt2 = $pdo->prepare("INSERT INTO notes (id, title, content, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($notes).' data copied (registed)'];
            break;
            case 'import':
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE notes SET id=?, title=?, content=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $title, $content, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in notes'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $unique = isUniqueLoop('notes',[$field=>$value]);

            $unique_cols = unique_cols('notes'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE notes SET `$field` = :value WHERE id = :id");
