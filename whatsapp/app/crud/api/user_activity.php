...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM user_activity WHERE id IN ($ids)");
    $user_activity = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=user_activity_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($user_activity[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all user_activity as CSV
            $stmt = $pdo->query("SELECT * FROM user_activity");
            $user_activity = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'user_activity_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($user_activity[0]));
            foreach($user_activity as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'user_id', 'activity_type', 'description', 'ip_address', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO user_activity (id, user_id, activity_type, description, ip_address, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'user_activity added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert user_activity");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `user_activity` SET  
                   id=?, user_id=?, activity_type=?, description=?, ip_address=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status, $id])) {
                $response = ['success'=>true, 'message'=>'user_activity updated successfully'];
            } else {
                throw new Exception("Failed to update user_activity");
            }
            break;
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM user_activity WHERE id=?");
            $stmt->execute([$id]);
            $user_activity = $stmt->fetch(PDO::FETCH_ASSOC);
            if($user_activity) $response = ['success'=>true, 'data'=>$user_activity];
            else throw new Exception("user_activity not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM user_activity WHERE id IN ($in)");
            $stmt->execute($ids);
            $user_activity = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$user_activity]);
            exit;
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `user_activity` SET  
                   id=?, user_id=?, activity_type=?, description=?, ip_address=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO user_activity (id, user_id, activity_type, description, ip_address, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'user_id', 'activity_type', 'description', 'ip_address', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO user_activity (id, user_id, activity_type, description, ip_address, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM user_activity WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' user_activity deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM user_activity WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $user_activity = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($user_activity as $u){
                $stmt2 = $pdo->prepare("INSERT INTO user_activity (id, user_id, activity_type, description, ip_address, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($user_activity).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO user_activity ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE user_activity SET id=?, user_id=?, activity_type=?, description=?, ip_address=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $user_id, $activity_type, $description, $ip_address, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in user_activity'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'user_id', 'activity_type', 'description', 'ip_address', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('user_activity',[$field=>$value]);

            $unique_cols = unique_cols('user_activity'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE user_activity SET `$field` = :value WHERE id = :id");
