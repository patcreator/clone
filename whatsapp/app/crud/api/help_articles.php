...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM help_articles WHERE id IN ($ids)");
    $help_articles = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=help_articles_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($help_articles[0])); // Header row
    foreach($help_articles as $row){
        fputcsv($output, $row);
    }
    fclose($output);
    exit;
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all help_articles as CSV
            $stmt = $pdo->query("SELECT * FROM help_articles");
            $help_articles = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'help_articles_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($help_articles[0]));
            foreach($help_articles as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'title', 'slug', 'category', 'content', 'keywords', 'is_active', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO help_articles (id, title, slug, category, content, keywords, is_active, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'help_articles added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert help_articles");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `help_articles` SET  
                   id=?, title=?, slug=?, category=?, content=?, keywords=?, is_active=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status, $id])) {
                $response = ['success'=>true, 'message'=>'help_articles updated successfully'];
            } else {
                throw new Exception("Failed to update help_articles");
            }
            break;
        ...
        case 'view':
            ...
            $stmt->execute([$id]);
            $help_articles = $stmt->fetch(PDO::FETCH_ASSOC);
            if($help_articles) $response = ['success'=>true, 'data'=>$help_articles];
            else throw new Exception("help_articles not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM help_articles WHERE id IN ($in)");
            $stmt->execute($ids);
            $help_articles = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$help_articles]);
            exit;
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `help_articles` SET  
                   id=?, title=?, slug=?, category=?, content=?, keywords=?, is_active=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO help_articles (id, title, slug, category, content, keywords, is_active, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'title', 'slug', 'category', 'content', 'keywords', 'is_active', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO help_articles (id, title, slug, category, content, keywords, is_active, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM help_articles WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' help_articles deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM help_articles WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $help_articles = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($help_articles as $u){
                $stmt2 = $pdo->prepare("INSERT INTO help_articles (id, title, slug, category, content, keywords, is_active, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($help_articles).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO help_articles ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE help_articles SET id=?, title=?, slug=?, category=?, content=?, keywords=?, is_active=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $title, $slug, $category, $content, $keywords, $is_active, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in help_articles'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'title', 'slug', 'category', 'content', 'keywords', 'is_active', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('help_articles',[$field=>$value]);

            $unique_cols = unique_cols('help_articles'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE help_articles SET `$field` = :value WHERE id = :id");
