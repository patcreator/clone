...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM blogs_and_categories WHERE BAC_ID IN ($ids)");
    $blogs_and_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=blogs_and_categories_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($blogs_and_categories[0])); // Header row
    foreach($blogs_and_categories as $row){
        fputcsv($output, $row);
    }
    fclose($output);
    exit;
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all blogs_and_categories as CSV
            $stmt = $pdo->query("SELECT * FROM blogs_and_categories");
            $blogs_and_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'blogs_and_categories_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($blogs_and_categories[0]));
            foreach($blogs_and_categories as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        case 'insert':
            ...
            $stmt = $pdo->prepare("INSERT INTO blogs_and_categories ( 
                    BAC_ID,
            ...
            if($stmt->execute([ 
                    ...
                    $created_at,])) {
                $response = ['success'=>true, 'message'=>'blogs_and_categories added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert blogs_and_categories");
            }
            break;
        ...
        case 'edit':
            ...
            $stmt = $pdo->prepare("UPDATE blogs_and_categories SET  
                   BAC_ID=?, 
            ...
            if($stmt->execute([ 
                    ...
                    $created_at,, BAC_ID])) {
                $response = ['success'=>true, 'message'=>'blogs_and_categories updated successfully'];
            } else {
                throw new Exception("Failed to update blogs_and_categories");
            }
            break;
        ...
        case 'delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM blogs_and_categories WHERE BAC_ID=?");
            if($stmt->execute([$id])) {
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM blogs_and_categories WHERE BAC_ID=?");
            $stmt->execute([$id]);
            $blogs_and_categories = $stmt->fetch(PDO::FETCH_ASSOC);
            if($blogs_and_categories) $response = ['success'=>true, 'data'=>$blogs_and_categories];
            else throw new Exception("blogs_and_categories not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM blogs_and_categories WHERE id IN ($in)");
            $stmt->execute($ids);
            $blogs_and_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$blogs_and_categories]);
            exit;
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM blogs_and_categories WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' blogs_and_categories deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM blogs_and_categories WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $blogs_and_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($blogs_and_categories as $u){
                $stmt2 = $pdo->prepare("INSERT INTO blogs_and_categories ( 
                    BAC_ID,
            ...
            $response = ['success'=>true, 'message'=>count($blogs_and_categories).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO blogs_and_categories ($columns) VALUES ($placeholders)");
        ...
        case 'update_many':
            ...
            $response = ['success'=>true, 'message'=>count($ids).' data updated in blogs_and_categories'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $unique = isUniqueLoop('blogs_and_categories',[$field=>$value]);

            $unique_cols = unique_cols('blogs_and_categories'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE blogs_and_categories SET `$field` = :value WHERE id = :id");
