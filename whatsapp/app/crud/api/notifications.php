...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM notifications WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($notifications[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all notifications as CSV
            ...
            fputcsv($out, array_keys($notifications[0]));
            foreach($notifications as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'user_id', 'sender_id', 'title', 'message', 'type', 'link', 'is_read', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO notifications (id, user_id, sender_id, title, message, type, link, is_read, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'notifications added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `notifications` SET  
                   id=?, user_id=?, sender_id=?, title=?, message=?, type=?, link=?, is_read=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status, $id])) {
        ...
        case 'view':
            ...
            if($notifications) $response = ['success'=>true, 'data'=>$notifications];
            else throw new Exception("notifications not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM notifications WHERE id IN ($in)");
            $stmt->execute($ids);
            $notifications = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `notifications` SET  
                   id=?, user_id=?, sender_id=?, title=?, message=?, type=?, link=?, is_read=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO notifications (id, user_id, sender_id, title, message, type, link, is_read, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'user_id', 'sender_id', 'title', 'message', 'type', 'link', 'is_read', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO notifications (id, user_id, sender_id, title, message, type, link, is_read, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' notifications deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM notifications WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $notifications = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($notifications as $u){
                $stmt2 = $pdo->prepare("INSERT INTO notifications (id, user_id, sender_id, title, message, type, link, is_read, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($notifications).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO notifications ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE notifications SET id=?, user_id=?, sender_id=?, title=?, message=?, type=?, link=?, is_read=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $user_id, $sender_id, $title, $message, $type, $link, $is_read, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in notifications'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'user_id', 'sender_id', 'title', 'message', 'type', 'link', 'is_read', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('notifications',[$field=>$value]);

            $unique_cols = unique_cols('notifications'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE notifications SET `$field` = :value WHERE id = :id");
