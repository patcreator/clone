...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM blog_categories WHERE blog_category_id IN ($ids)");
    $blog_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=blog_categories_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($blog_categories[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all blog_categories as CSV
            $stmt = $pdo->query("SELECT * FROM blog_categories");
            $blog_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'blog_categories_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($blog_categories[0]));
            foreach($blog_categories as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        case 'insert':
                    ...
                    $id = trim($_POST['blog_category_id'] ?? '');
                 
                    $id = trim($_POST['blog_category_name'] ?? '');
                 
                    $id = trim($_POST['blog_category_status'] ?? '');
                 
                    $id = trim($_POST['blog_category_slug'] ?? '');
            ...
            $stmt = $pdo->prepare("INSERT INTO blog_categories ( 
                    blog_category_id,
            ...
            if($stmt->execute([ 
                    ...
                    $created_at,])) {
                $response = ['success'=>true, 'message'=>'blog_categories added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert blog_categories");
            }
            break;
        ...
        case 'edit':
            ...
            $stmt = $pdo->prepare("UPDATE blog_categories SET  
                   ...
                   created_at=?, WHERE blog_category_id=?");
            if($stmt->execute([ 
                    $blog_category_id,
                    ...
                    $created_at,, blog_category_id])) {
                $response = ['success'=>true, 'message'=>'blog_categories updated successfully'];
            } else {
                throw new Exception("Failed to update blog_categories");
            }
            break;
        ...
        case 'delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM blog_categories WHERE blog_category_id=?");
            if($stmt->execute([$id])) {
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM blog_categories WHERE blog_category_id=?");
            $stmt->execute([$id]);
            $blog_categories = $stmt->fetch(PDO::FETCH_ASSOC);
            if($blog_categories) $response = ['success'=>true, 'data'=>$blog_categories];
            else throw new Exception("blog_categories not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM blog_categories WHERE id IN ($in)");
            $stmt->execute($ids);
            $blog_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$blog_categories]);
            exit;
        ...
        case 'bulk_delete':
            ...
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' blog_categories deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM blog_categories WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $blog_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($blog_categories as $u){
                $stmt2 = $pdo->prepare("INSERT INTO blog_categories ( 
                    blog_category_id,
                ...
                $u[\'blog_category_name\'],
                 
                $u[\'blog_category_status\'],
                 
                $u[\'blog_category_slug\'],
                 
                $u[\'created_at\'],]);
            }
            $response = ['success'=>true, 'message'=>count($blog_categories).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO blog_categories ($columns) VALUES ($placeholders)");
        ...
        case 'update_many':
            ...
            $response = ['success'=>true, 'message'=>count($ids).' data updated in blog_categories'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $unique = isUniqueLoop('blog_categories',[$field=>$value]);

            $unique_cols = unique_cols('blog_categories'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE blog_categories SET `$field` = :value WHERE id = :id");
