...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM contacts WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($contacts[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            fputcsv($out, array_keys($contacts[0]));
            foreach($contacts as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'name', 'phone', 'email', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO contacts (id, name, phone, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $name, $phone, $email, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'contacts added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert contacts");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `contacts` SET  
                   id=?, name=?, phone=?, email=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $name, $phone, $email, $created_at, $updated_at, $status, $id])) {
                $response = ['success'=>true, 'message'=>'contacts updated successfully'];
            } else {
                throw new Exception("Failed to update contacts");
            }
            break;
        ...
        case 'view':
            ...
            if($contacts) $response = ['success'=>true, 'data'=>$contacts];
            else throw new Exception("contacts not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM contacts WHERE id IN ($in)");
            $stmt->execute($ids);
            $contacts = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `contacts` SET  
                   id=?, name=?, phone=?, email=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $name, $phone, $email, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO contacts (id, name, phone, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $name, $phone, $email, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'name', 'phone', 'email', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO contacts (id, name, phone, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $name, $phone, $email, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' contacts deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM contacts WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $contacts = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($contacts as $u){
                $stmt2 = $pdo->prepare("INSERT INTO contacts (id, name, phone, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($contacts).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO contacts ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE contacts SET id=?, name=?, phone=?, email=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $name, $phone, $email, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in contacts'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'name', 'phone', 'email', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('contacts',[$field=>$value]);

            $unique_cols = unique_cols('contacts'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE contacts SET `$field` = :value WHERE id = :id");
