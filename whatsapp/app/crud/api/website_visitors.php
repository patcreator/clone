...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM website_visitors WHERE id IN ($ids)");
    $website_visitors = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=website_visitors_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($website_visitors[0])); // Header row
    foreach($website_visitors as $row){
        fputcsv($output, $row);
    }
    fclose($output);
    exit;
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all website_visitors as CSV
            $stmt = $pdo->query("SELECT * FROM website_visitors");
            $website_visitors = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'website_visitors_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($website_visitors[0]));
            foreach($website_visitors as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        case 'insert':
            ...
            $stmt = $pdo->prepare("INSERT INTO website_visitors ( 
                    id,
            ...
            if($stmt->execute([ 
                    ...
                    $status,])) {
                $response = ['success'=>true, 'message'=>'website_visitors added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert website_visitors");
            }
            break;
        ...
        case 'edit':
            ...
            $stmt = $pdo->prepare("UPDATE website_visitors SET  
                   id=?, 
            ...
            if($stmt->execute([ 
                    ...
                    $status, $id])) {
                $response = ['success'=>true, 'message'=>'website_visitors updated successfully'];
            } else {
                throw new Exception("Failed to update website_visitors");
            }
            break;
        ...
        case 'delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM website_visitors WHERE id=?");
            if($stmt->execute([$id])) {
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM website_visitors WHERE id=?");
            $stmt->execute([$id]);
            $website_visitors = $stmt->fetch(PDO::FETCH_ASSOC);
            if($website_visitors) $response = ['success'=>true, 'data'=>$website_visitors];
            else throw new Exception("website_visitors not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM website_visitors WHERE id IN ($in)");
            $stmt->execute($ids);
            $website_visitors = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$website_visitors]);
            exit;
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM website_visitors WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' website_visitors deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM website_visitors WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $website_visitors = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($website_visitors as $u){
                $stmt2 = $pdo->prepare("INSERT INTO website_visitors ( 
                    id,
            ...
            $response = ['success'=>true, 'message'=>count($website_visitors).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO website_visitors ($columns) VALUES ($placeholders)");
        ...
        case 'update_many':
            ...
            $response = ['success'=>true, 'message'=>count($ids).' data updated in website_visitors'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $unique = isUniqueLoop('website_visitors',[$field=>$value]);

            $unique_cols = unique_cols('website_visitors'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE website_visitors SET `$field` = :value WHERE id = :id");
