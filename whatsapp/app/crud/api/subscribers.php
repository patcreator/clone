...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM subscribers WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($subscribers[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            $subscribers = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'subscribers_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($subscribers[0]));
            foreach($subscribers as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $email, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'subscribers added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert subscribers");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `subscribers` SET  
                   id=?, email=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $email, $created_at, $updated_at, $status, $id])) {
                $response = ['success'=>true, 'message'=>'subscribers updated successfully'];
            } else {
                throw new Exception("Failed to update subscribers");
            }
            break;
        ...
        case 'view':
            ...
            if($subscribers) $response = ['success'=>true, 'data'=>$subscribers];
            else throw new Exception("subscribers not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM subscribers WHERE id IN ($in)");
            $stmt->execute($ids);
            $subscribers = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `subscribers` SET  
                   id=?, email=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $email, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$id, $email, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'email', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$id, $email, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' subscribers deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM subscribers WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $subscribers = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($subscribers as $u){
                $stmt2 = $pdo->prepare("INSERT INTO subscribers (id, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($subscribers).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO subscribers ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE subscribers SET id=?, email=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $email, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in subscribers'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'email', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('subscribers',[$field=>$value]);

            $unique_cols = unique_cols('subscribers'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE subscribers SET `$field` = :value WHERE id = :id");
