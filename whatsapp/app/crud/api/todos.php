...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM todos WHERE id IN ($ids)");
    $todos = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=todos_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($todos[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            $todos = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'todos_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($todos[0]));
            foreach($todos as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'task', 'completed', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO todos (id, task, completed, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $task, $completed, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'todos added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert todos");
            }
            break;
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `todos` SET  
                   id=?, task=?, completed=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $task, $completed, $created_at, $updated_at, $status, $id])) {
                $response = ['success'=>true, 'message'=>'todos updated successfully'];
            } else {
                throw new Exception("Failed to update todos");
            }
            break;
        ...
        case 'view':
            ...
            if($todos) $response = ['success'=>true, 'data'=>$todos];
        ...
        case 'bulk_view':
            ...
            $stmt = $pdo->prepare("SELECT * FROM todos WHERE id IN ($in)");
            $stmt->execute($ids);
            $todos = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `todos` SET  
                   id=?, task=?, completed=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $task, $completed, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO todos (id, task, completed, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $task, $completed, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'task', 'completed', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO todos (id, task, completed, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $task, $completed, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM todos WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' todos deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM todos WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $todos = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($todos as $u){
                $stmt2 = $pdo->prepare("INSERT INTO todos (id, task, completed, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($todos).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO todos ($columns) VALUES ($placeholders)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE todos SET id=?, task=?, completed=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $task, $completed, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in todos'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'task', 'completed', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('todos',[$field=>$value]);

            $unique_cols = unique_cols('todos'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE todos SET `$field` = :value WHERE id = :id");
