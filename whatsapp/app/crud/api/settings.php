...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM settings WHERE id IN ($ids)");
    ...
    fputcsv($output, array_keys($settings[0])); // Header row
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            ...
            $settings = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'settings_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($settings[0]));
            foreach($settings as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot([ 
                    'id', 'name', 'value', 'type', 'created_at', 'updated_at', 'status'
                ]);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO settings (id, name, value, type, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $name, $value, $type, $created_at, $updated_at, $status])) {
                $response = ['success'=>true, 'message'=>'settings added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE `settings` SET  
                   id=?, name=?, value=?, type=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
            if($stmt->execute([$id, $name, $value, $type, $created_at, $updated_at, $status, $id])) {
        ...
        case 'view':
            ...
            if($settings) $response = ['success'=>true, 'data'=>$settings];
        ...
        case 'bulk_view':
            ...
            $stmt = $pdo->prepare("SELECT * FROM settings WHERE id IN ($in)");
            $stmt->execute($ids);
            $settings = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE `settings` SET  
                   id=?, name=?, value=?, type=?, created_at=?, updated_at=?, status=?
                    WHERE `id`=?");
               $stmt->execute([$id, $name, $value, $type, $created_at, $updated_at, $status, $id]) 
            }
        ...
        case 'copy':
            ...
            $stmt = $pdo->prepare("INSERT INTO settings (id, name, value, type, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$id, $name, $value, $type, $created_at, $updated_at, $status]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                isNot(['id', 'name', 'value', 'type', 'created_at', 'updated_at', 'status']);
                $stmt = $pdo->prepare("INSERT INTO settings (id, name, value, type, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $name, $value, $type, $created_at, $updated_at, $status]);
            }
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM settings WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' settings deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM settings WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $settings = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($settings as $u){
                $stmt2 = $pdo->prepare("INSERT INTO settings (id, name, value, type, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt2->execute([$u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email'], $u['email']]);
            }
            $response = ['success'=>true, 'message'=>count($settings).' data copied (registed)'];
            break;
            case 'import':
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE settings SET id=?, name=?, value=?, type=?, created_at=?, updated_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $name, $value, $type, $created_at, $updated_at, $status], $ids));
            $response = ['success'=>true, 'message'=>count($ids).' data updated in settings'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $allowed_fields = ['id', 'name', 'value', 'type', 'created_at', 'updated_at', 'status'];
            if (!in_array($field, $allowed_fields)) {
                echo json_encode(['success' => false, 'message' => 'Invalid field']);
                exit;
            }

            $unique = isUniqueLoop('settings',[$field=>$value]);

            $unique_cols = unique_cols('settings'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE settings SET `$field` = :value WHERE id = :id");
