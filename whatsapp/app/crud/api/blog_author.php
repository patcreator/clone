...
if(isset($_GET['action']) && $_GET['action']=='bulk_export' && !empty($_GET['ids'])){
    ...
    $stmt = $pdo->query("SELECT * FROM blog_author WHERE blog_author_id IN ($ids)");
    $blog_author = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Export CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename=blog_author_export.csv');
    $output = fopen('php://output','w');
    fputcsv($output, array_keys($blog_author[0])); // Header row
    foreach($blog_author as $row){
        fputcsv($output, $row);
    }
    fclose($output);
    exit;
...
if (isset($_GET['action']) && $_GET['action']=='export'){
            // export all blog_author as CSV
            $stmt = $pdo->query("SELECT * FROM blog_author");
            $blog_author = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $filename = 'blog_author_export_'.date('Ymd_His').'.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="'.$filename.'"');
            $out = fopen('php://output', 'w');
            fputcsv($out, array_keys($blog_author[0]));
            foreach($blog_author as $u) fputcsv($out, $u);
            fclose($out);
            exit;
...
try {
    switch($action) {
        ...
        case 'insert':
                    ...
                    $id = trim($_POST['blog_author_id'] ?? '');
                 
                    $id = trim($_POST['blog_author_name'] ?? '');
                 
                    $id = trim($_POST['blog_author_img'] ?? '');
                 
                    $id = trim($_POST['blog_author_bio'] ?? '');
                 
                    $id = trim($_POST['blog_author_email'] ?? '');
                 
                    $id = trim($_POST['blog_author_password'] ?? '');
                 
                    $id = trim($_POST['blog_author_phone'] ?? '');
                 
                    $id = trim($_POST['blog_author_status'] ?? '');
            ...
            $stmt = $pdo->prepare("INSERT INTO blog_author ( 
                    blog_author_id,
            ...
            if($stmt->execute([ 
                    ...
                    $created_at,])) {
                $response = ['success'=>true, 'message'=>'blog_author added successfully', 'id'=>$pdo->lastInsertId()];
            } else {
                throw new Exception("Failed to insert blog_author");
            }
            break;
        ...
        case 'edit':
            ...
            $stmt = $pdo->prepare("UPDATE blog_author SET  
                   blog_author_id=?, 
                   ...
                   created_at=?, WHERE blog_author_id=?");
            if($stmt->execute([ 
                    $blog_author_id,
                    ...
                    $created_at,, blog_author_id])) {
                $response = ['success'=>true, 'message'=>'blog_author updated successfully'];
            } else {
                throw new Exception("Failed to update blog_author");
            }
            break;
        ...
        case 'delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM blog_author WHERE blog_author_id=?");
            if($stmt->execute([$id])) {
        ...
        case 'view':
            $id = intval($_POST['id'] ?? 'unknow');
            if($id == 'unknow') throw new Exception("Invalid ID");
            $stmt = $pdo->prepare("SELECT * FROM blog_author WHERE blog_author_id=?");
            $stmt->execute([$id]);
            $blog_author = $stmt->fetch(PDO::FETCH_ASSOC);
            if($blog_author) $response = ['success'=>true, 'data'=>$blog_author];
            else throw new Exception("blog_author not found");
            break;

        case 'bulk_view':
            $ids = $_POST['ids']; // array of IDs
            $in  = str_repeat('?,', count($ids) - 1) . '?';
            $stmt = $pdo->prepare("SELECT * FROM blog_author WHERE id IN ($in)");
            $stmt->execute($ids);
            $blog_author = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode(['success'=>true, 'data'=>$blog_author]);
            exit;
        ...
        case 'bulk_delete':
            ...
            $stmt = $pdo->prepare("DELETE FROM blog_author WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $response = ['success'=>true, 'message'=>count($ids).' blog_author deleted'];
            break;

        case 'copy_many':
            if(empty($ids)) throw new Exception("No IDs provided");
            $stmt = $pdo->prepare("SELECT * FROM blog_author WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute($ids);
            $blog_author = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($blog_author as $u){
                $stmt2 = $pdo->prepare("INSERT INTO blog_author ( 
                    blog_author_id,
                ...
                $u[\'blog_author_password\'],
                 
                $u[\'blog_author_phone\'],
                 
                $u[\'blog_author_status\'],
                 
                $u[\'created_at\'],]);
            }
            $response = ['success'=>true, 'message'=>count($blog_author).' data copied (registed)'];
            break;
            case 'import':
                    ...
                    if ($ext === 'csv') {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO blog_author ($columns) VALUES ($placeholders)");
        ...
        case 'update_many':
            ...
            $response = ['success'=>true, 'message'=>count($ids).' data updated in blog_author'];
            break;
        case 'inline_update':
            $id = intval($_POST['id']);
            $field = $_POST['field'];
            ...
            $unique = isUniqueLoop('blog_author',[$field=>$value]);

            $unique_cols = unique_cols('blog_author'); 
            ...
            if ($unique) {
                ...
                $stmt = $pdo->prepare("UPDATE blog_author SET `$field` = :value WHERE id = :id");
