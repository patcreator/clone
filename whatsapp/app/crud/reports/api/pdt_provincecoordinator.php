<?php
// Auto-generated report API for table: pdt_provincecoordinator
...
if (isset($_GET['ajax'])) {
    ...
    $sql = "SELECT `id`, `fullName`, `email`, `phone`, `password`, `createdDate`, `status`, `Level`, `Location`, `created_at` FROM `pdt_provincecoordinator` WHERE 1";
    $params = [];
    if ($query) {
        $likeParts = [];
        $likeParts[] = "fullName LIKE ?";
        $likeParts[] = "email LIKE ?";
        $likeParts[] = "phone LIKE ?";
        $likeParts[] = "password LIKE ?";
        $likeParts[] = "Level LIKE ?";
    ...
    $allowedSort = ['id','createdDate','created_at'];
