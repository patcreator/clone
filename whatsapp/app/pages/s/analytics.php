...
?>
  ...
  <title><?= $site_name ?> . Analytics . <?= $table ?></title>
...
<div class="max-w-6xl mx-auto">
  <header class="flex justify-between ms-9 items-center mb-6">
    <div>
      <h1 class="text-2xl font-semibold">Analytics Center</h1>
      <p class="text-sm text-slate-500">Choose a table to view its analytics.</p>
    </div>
  </header>

  <!-- Table Selector -->
  <div class="flex gap-3 mb-6 ms-9">
    <select onchange="location.href='<?= $path?>s/Analytics?get='+this.value" id="tableSelector" class="px-4 py-2 rounded-lg border bg-white dark:bg-slate-800 dark:border-slate-700 text-sm">
  ...
  <!-- Content Area -->
  <div id="contentArea">
    <?php if (file_exists('app/crud/analytics/'.$table.'.php')) include_once 'app/crud/analytics/'.$table.'.php'; ?>
  </div>
</div>
</section>